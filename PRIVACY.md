...
アプリ内の「プロジェクトをリセット」は現在の制作状態を空にします。アプリをアンインストールするとアプリ専用領域は Android により削除されます。ファイル選択画面から書き出した WAV や `.choplab` は自動削除されないため、保存先のファイルアプリで管理してください。
