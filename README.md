...
現在は、曲を流しながら16 PADを叩いてその瞬間を刻むライブチョップ、音声の取り込み、PAD別トーン、4 BANK、実波形と再生位置を表示する16ステップシーケンス、4つ打ち・8分・16分の反復配置、WAV書き出し、WAV音声を内包する`.choplab`制作保存、二世代自動保存、40操作のUndo/Redoを備えたMVPと、次段階へ移行するためのステレオ対応ドメイン基盤を公開しています。GitHub Releasesには、タグから自動生成する開発プレビューAPKを添付します。
...
操作の中心となる16 PADと`ALL STOP`は各制作工程で位置関係を保ちます。`並べる`では選択PADの実波形、太い再生ヘッド、A〜D BANKの発音マーカーを一画面で確認し、反復プリセットからすぐビートを作って別BANKの音を重ねられます。`叩く`と`並べる`では選択中PADのKEY、意味名付きTONE、LEVELを画面内で調整でき、reverse、gate、chokeなどは`叩く`の詳細編集から到達できます。破壊操作は確認を挟み、押下状態、選択状態、再生・録音状態を文字と色の両方で示します。
