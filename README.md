...
画面は、クリーム色のデッキ、オレンジのサンプリング表示、緑の波形、4 × 4 PADを中心とするオリジナルの「おとひろい」UIです。縦横どちらでも画面スクロールを使わず、`CHOP / PAD / SEQ / SOURCE` の固定4モードから演奏、PAD編集、シーケンス、音源編集へ直接移動できます。

操作の中心となる16 PADと`ALL STOP`は各制作モードで位置関係を保ちます。破壊操作は確認を挟み、押下状態、選択状態、再生・録音状態を文字と色の両方で示します。
