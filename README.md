...
現在は、曲を流しながら16 PADを叩いてその瞬間を刻むライブチョップ、音声の取り込み、PAD別トーン、4 BANK、チョップ済みビート音声全体の連続ループ、実波形上のループ再生位置、別PAD用の4つ打ち・8分・16分配置プリセット、WAV書き出し、WAV音声を内包する`.choplab`制作保存、二世代自動保存、40操作のUndo/Redoを備えたMVPと、次段階へ移行するためのステレオ対応ドメイン基盤を公開しています。GitHub Releasesには、タグから自動生成する開発プレビューAPKを添付します。

画面は、クリーム色のデッキ、オレンジのサンプリング表示、緑の波形、4 × 4 PADを中心とするオリジナルの「おとひろい」UIです。縦横どちらでも画面スクロールを使わず、`入れる / 切る / 叩く / 並べる / 完成` の固定5工程から取込、精密編集、ライブチョップ、16-step制作、WAV書き出しへ直接移動できます。

操作の中心となる16 PADと`ALL STOP`は各制作工程で位置関係を保ちます。`並べる`では`PADを選ぶ → ビートをループ → 音を重ねる`の3操作を常時表示し、チョップ範囲の末尾から先頭へ戻る再生位置を実波形上で確認できます。4つ打ち・8分・16分はループとは別の`配置プリセット`として細かい調整に残し、別BANKのドラムを重ねられます。`叩く`と`並べる`では選択中PADのKEY、意味名付きTONE、LEVELを画面内で調整でき、reverse、gate、chokeなどは`叩く`の詳細編集から到達できます。破壊操作は確認を挟み、押下状態、選択状態、再生・録音状態を文字と色の両方で示します。
