...
class PublicationContractTests(unittest.TestCase):
    ...
    def test_callback_contract_applies_even_without_allocation_tests(self) -> None:
        text = (ROOT / "docs/DEFINITION_OF_DONE.md").read_text(encoding="utf-8")
        self.assertIn("Every audio callback and high-frequency DSP inner path", text)
        self.assertIn("regardless of existing test coverage", text)
        for prohibition in ("heap allocation or deallocation", "blocking locks", "file I/O", "logging",
                            "Android UI APIs", "heavy JNI work"):
            self.assertIn(prohibition, text)
        self.assertNotIn("Callback paths covered by the allocation-free contract", text)
        self.assertNotIn("unbounded allocation", text)
