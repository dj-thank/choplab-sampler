...
object DesktopMediaRuntime {
    fun directory(): File = listOfNotNull(
        ...
        File("work/media-tools"),File("../work/media-tools"),
    ).firstOrNull { listOf("yt-dlp.exe","ffmpeg.exe","ffprobe.exe","node.exe").all { name ->File(it,name).isFile } }
        ?: error("取り込み用ツールがありません。ChopLabのアプリ一式を使用してください")
}
...
class DesktopYoutubeBackend : YoutubeSourceBackend {
    ...
    private fun yt(arguments:List<String>,id:String,progress:(Float)->Unit = {}):String {
        val tools=DesktopMediaRuntime.directory()
        return run(File(tools,"yt-dlp.exe"), listOf("--ffmpeg-location",tools.absolutePath,"--js-runtimes","node:${File(tools,"node.exe").absolutePath}")+arguments,id,progress)
    }
...
object DesktopAudioDecoder {
    fun decode(file:File):PcmAudio {
        ...
        val tools=DesktopMediaRuntime.directory(); val runner=DesktopYoutubeBackend();val id=UUID.randomUUID().toString()
        val duration=runner.run(File(tools,"ffprobe.exe"),listOf("-v","error","-protocol_whitelist","file,pipe","-show_entries","format=duration","-of","default=noprint_wrappers=1:nokey=1",file.absolutePath),id).trim().toDoubleOrNull()
        ...
        try {
            runner.run(File(tools,"ffmpeg.exe"),listOf("-nostdin","-hide_banner","-loglevel","error","-protocol_whitelist","file,pipe","-i",file.absolutePath,"-map","0:a:0","-vn","-c:a","pcm_s16le","-ar","48000","-ac","2","-y",temporary.absolutePath),id)
