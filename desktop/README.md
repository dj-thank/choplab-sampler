...
The desktop app supports user-selected WAV import, microphone recording, a driver-exposed Windows playback loopback such as `Stereo Mix`, PAD voice controls, 16-step transport, scratch, four-bar WAV export, Undo/Redo, manual `.choplab` save/open, and app-owned three-generation autosave. Closing captures the latest project snapshot, stops live audio, then drains one scheduled save or waits for the already-running save without duplicating a recovery generation. If that running save owns an older project revision, the newer close snapshot is persisted once after it completes. A loopback input is never silently replaced with a microphone; unsupported drivers return a visible error.
