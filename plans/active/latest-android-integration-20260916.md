...
- The third, `normalTextGatePointerUpCannotCutANewerControllerTrigger`, is an open question about the app, not about the test harness. Pressing an assigned GATE pad in the chop stage records `capturePad` at normal text and `triggerPad` at large text: `PerformancePad.onPress` routes through `triggerOwnedGate`, which captures while `captureMode` is on, whereas the large-text path (`deferPadActionUntilTap`) calls `onTrigger` from `onTap` and always auditions. The recorded actions were `[selectPad, capturePad, releasePad, triggerPad]`. Which side is right decides whether the test or the deck changes; left for the owner.
