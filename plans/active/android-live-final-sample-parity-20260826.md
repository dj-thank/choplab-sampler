...
- Defect seam: `SamplerEngine.renderLoop` obtains `value = voice.render(outputSampleRate)`. When that call marks `finished`, current code deactivates the voice and skips `monoMix += value`. `PatternRenderer` mixes then retires, and the existing parity test's expected path also mixes the return.
...
- [ ] Full gate, reviews, artifact read-back and closeout.
