...
- [ ] 2026-08-10 - Complete fixed-point review, commit, PR/CI, and public preview release.
...
- `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug --offline --no-daemon`: PASS; 52 tests, zero failures/errors/skips; lint zero errors and nine existing advisories.
- `git diff --check`: PASS; source scan found zero scroll API matches.
- Final local APK: 30,942,730 bytes; SHA-256 `91870DBF844933B913019FE41CF04F743ADE72D18F5B5AE351464C457A045CF7`.
- Pixel 9a: exact APK installed as v0.7.0/code 8; exact bytes copied to Download; A-04 loop cursor advanced from 23% to 75%; active waveform/STOP remained visible after BANK B selection; focused fatal-log matches zero.
