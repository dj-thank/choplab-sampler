...
- [x] 2026-08-24 follow-up — moved the reviewed Android display-name rule to shared production code and applied it to Desktop `File.name` before PCM publication. Shared common tests bind candidate priority, post-truncation blank fallback and surrogate safety; a Desktop archive read-back regression binds the data-loss seam. Integration product `e0d3fa1` / tree `1558c4a` joins the prior clean head with merged `main@3072eed` while preserving #65 transport and #68 runtime/test blobs; fresh hosted Android and Windows CI are the executable gates.
- [x] 2026-08-24 follow-up — made Desktop stop-during-start fail closed. Product repair `27283f8` / tree `4ffc5f7` on exact-base `main@dfcd9d8` lets STOP claim/close the pending line while `TargetDataLine.start()` is blocked, prevents late worker publication/read, preserves exact-once close and removes the partial WAV. A deterministic close-driven latch regression binds the lifecycle seam; hosted Windows CI is the executable gate.
- [x] 2026-08-24 follow-up — bounded reverse PAD host rendering with realtime cursor advance order. Product `1b8b424` / tree `614c93f` rooted at exact merged `main@a0b356c` removes the fractional one-shot/gate silent tail and binds the supported 1/12-step rounding boundary. The review's non-integral pitch −5 fixture requires reverse LOOP and Windows `forceLoop` to retain main's direct-position three-element boundary and Desktop playhead contract; arbitrary-pitch loop phase remains separate work. Count-before-allocation/reset avoids a trim-copy; realtime and forward paths remain unchanged. Hosted JVM/Windows CI is the executable gate.
...
- Reverse PAD resampling repair `1b8b424` / tree `614c93f` — `[100, 164)` at 48→60 kHz produces 79 cursor positions instead of the old 80-frame allocation with one out-of-range zero tail. Count and PCM passes share realtime's cursor order; allocation follows exact counting/reset rather than trimming. Review regressions bind the two-frame 1/12-step one-shot to 12 frames and exercise the reported two-frame pitch −5 shape: one-shot returns 2 frames while LOOP/Windows `forceLoop` retain main's three-element finite render plus terminal boundary zero, without introducing cursor wrap or changing the monotonic Desktop playhead. Arbitrary-pitch loop phase continuity is not claimed. Python policy 39/39, public-surface current/history over 396 candidates, conflict-marker scan and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted JVM/Windows execution and exact-head review are required.

- Desktop recorder startup cancellation repair `27283f8` / tree `4ffc5f7` — latch regression blocks `TargetDataLine.start()`, issues STOP and requires pending-line close to release the gate, followed by exact cancellation, zero reads, exact-once line close, idle state and partial-file deletion. Python policy 39/39, public-surface current/history over 395 candidates and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted Windows execution and exact-head review are required.

- Shared Android/Desktop import-name integration `e0d3fa1` / tree `1558c4a` — prior head `d2c99fc` passed clean review and all four workflows before main advanced. Latest-main local Python 39/39, public-surface 395, exact #65/#68/product blob checks, conflict-marker scan and `git diff --check` PASS. Uncached Gradle 9.7.1 remains unavailable locally; fresh hosted execution is required.
