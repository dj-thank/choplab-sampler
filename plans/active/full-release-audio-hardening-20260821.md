...
- [x] 2026-08-24 follow-up — bounded reverse PAD host rendering to positions reachable from the last included source frame. Product `6bb8045` / tree `f2f9912` on exact merged `main@a0b356c` removes the extra out-of-range silent tail at fractional resampling rates; a 48→60 kHz cursor-parity regression fixes the 79-frame contract while leaving realtime and forward paths unchanged. Hosted JVM/Windows CI is the executable gate.
...
- Reverse PAD resampling repair `6bb8045` / tree `f2f9912` — `[100, 164)` at 48→60 kHz produces 79 valid reverse positions instead of the old 80-frame allocation with one out-of-range zero tail. The focused regression advances the realtime cursor independently and requires equal length plus a nonzero last valid PCM. Python policy 39/39, public-surface current/history over 395 candidates, conflict-marker scan and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted JVM/Windows execution and exact-head review are required.
