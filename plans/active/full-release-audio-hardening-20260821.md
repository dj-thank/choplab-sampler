...
- [x] 2026-08-24 follow-up — bounded reverse PAD host rendering with realtime cursor advance order. Product `830e12a` / tree `6d9f921` rooted at exact merged `main@a0b356c` removes the fractional one-shot silent tail, binds the supported 1/12-step rounding boundary, and preserves the reverse LOOP full-range cycle. Count-before-allocation/reset avoids a trim-copy; realtime and forward paths remain unchanged. Hosted JVM/Windows CI is the executable gate.
