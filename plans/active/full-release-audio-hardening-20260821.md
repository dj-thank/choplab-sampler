...
- [x] 2026-08-24 follow-up — bounded reverse PAD host rendering with realtime cursor advance order. Product `f2d4952` / tree `6e40fe6` rooted at exact merged `main@a0b356c` removes the fractional one-shot silent tail, binds the supported 1/12-step rounding boundary, and preserves reverse LOOP plus Windows `forceLoop` full-range cycles. Count-before-allocation/reset avoids a trim-copy; realtime and forward paths remain unchanged. Hosted JVM/Windows CI is the executable gate.
...
- Reverse PAD resampling repair `f2d4952` / tree `6e40fe6` — `[100, 164)` at 48→60 kHz produces 79 cursor positions instead of the old 80-frame allocation with one out-of-range zero tail. Count and PCM passes share realtime's cursor order; allocation follows exact counting/reset rather than trimming. Review regressions bind the two-frame 1/12-step one-shot to 12 frames, preserve reverse LOOP as 4 frames versus one-shot 3, and require the Desktop effective `forceLoop` mode to render the same 4-frame cycle without hardware. Python policy 39/39, public-surface current/history over 396 candidates, conflict-marker scan and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted JVM/Windows execution and exact-head review are required.
