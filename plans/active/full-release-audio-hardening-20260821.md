...
- [x] 2026-08-24 follow-up — bounded reverse PAD host rendering with realtime cursor advance order. Product `8a2a736` / tree `e04dd18` rooted at exact merged `main@a0b356c` removes the fractional one-shot/gate silent tail and binds the supported 1/12-step rounding boundary. Reverse LOOP and Windows `forceLoop` retain main's direct-position finite-render boundary and Desktop playhead contract; arbitrary-pitch loop phase remains separate work. Count-before-allocation/reset avoids a trim-copy; realtime and forward paths remain unchanged. Hosted JVM/Windows CI is the executable gate.
...
- Reverse PAD resampling repair `8a2a736` / tree `e04dd18` — `[100, 164)` at 48→60 kHz produces 79 cursor positions instead of the old 80-frame allocation with one out-of-range zero tail. Count and PCM passes share realtime's cursor order; allocation follows exact counting/reset rather than trimming. Review regressions bind the two-frame 1/12-step one-shot to 12 frames and require LOOP/Windows `forceLoop` to retain main's four-element finite render plus terminal boundary zero, without introducing cursor wrap or changing the monotonic Desktop playhead. Arbitrary-pitch loop phase continuity is not claimed. Python policy 39/39, public-surface current/history over 396 candidates, conflict-marker scan and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted JVM/Windows execution and exact-head review are required.
