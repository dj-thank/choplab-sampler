...
- [x] 2026-08-24 follow-up — made Desktop stop-during-start fail closed. Product `c8caf5d` / tree `b09f94e` on exact merged `main@dfcd9d8` latches STOP while `TargetDataLine.start()` is blocked, prevents late worker publication/read, closes the acquired line and removes the partial WAV. A deterministic latch regression binds the lifecycle seam; hosted Windows CI is the executable gate.
...
- Desktop recorder startup cancellation `c8caf5d` / tree `b09f94e` — latch regression blocks `TargetDataLine.start()`, issues STOP and requires exact cancellation, zero reads, exact-once line close, idle state and partial-file deletion. Python policy 39/39, public-surface current/history over 395 candidates and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted Windows execution and exact-head review are required.
