...
- [x] 2026-08-24 follow-up — bounded reverse PAD host rendering with realtime's one-shot cursor advance order. Product `6151317` / tree `73d6f68` rooted at exact merged `main@a0b356c` removes the extra out-of-range silent tail at fractional resampling rates; a 48→60 kHz cursor-parity regression fixes the 79-frame contract while leaving realtime and forward paths unchanged. The count-before-allocation/reset pass avoids a trim-copy; hosted JVM/Windows CI is the executable gate.
...
- Reverse PAD resampling repair `6151317` / tree `73d6f68` — `[100, 164)` at 48→60 kHz produces 79 cursor positions instead of the old 80-frame allocation with one out-of-range zero tail. Count and PCM passes share realtime's cursor advance order; allocation follows exact counting and reset rather than trimming a larger array. The focused regression requires equal realtime/host length plus a nonzero last valid PCM. Python policy 39/39, public-surface current/history over 395 candidates, conflict-marker scan and `git diff --check` PASS. Uncached Gradle 9.7.1 is unreachable locally; fresh hosted JVM/Windows execution and exact-head review are required.
