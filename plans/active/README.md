...
**独立したDesktop永続化修正:** 到達可能なproduct checkpoint `599e756`（tree `61ed25f`、`main@6b645ca`）は、window/controller close時に最終autosaveを失う競合を修正する限定follow-up。closeはadmitted project publicationをinvalidate後にdrainして最新snapshotを取得し、待機前にtransport／scratch／recorder／playerを停止する。scheduled workは一度だけflush、successful running workは重複送信せず待ち、failed latest workは一度retry、新revisionは旧成功work後に一度だけ追随保存する。上記画面planの選択、archive schema、三世代recovery policyは変更しない。
