...
**独立したAndroid microphone開始取消修正:** exact `main@a0b356c`上のproduct `8ffc1f4` / tree `4eee53d`で、blocking `AudioRecord.startRecording()`をlifecycle lock外へ移し、STOPがpending inputをclaimして`stop`/`release`できるようにする。exact-identity cleanupとstart/stop generation guardでlate worker、二重release、取消中replacementを阻止し、stop/release各1、read 0、一時WAV削除をlatch regressionへ固定する。既存planは再選択せず、hosted Android CIを実行gateとする。
