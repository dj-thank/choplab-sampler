...
**独立release-policy delta:** merged `main@dfcd9d8`を保持するreachable policy product `f3fe80c` / tree `56a3a43`は、公開面scanをcurrent／reachable-history ZIP member/comment/extra fieldへ拡張する小規模修正。byte-preserving scanとcase-insensitive per-parent raw path/blob履歴でZIP alias／merge-resultを列挙し、tree/symlink/gitlinkをblob capへ含めず、4096 entries、512 KiB/member、4 MiB/archive、100:1圧縮率と履歴blob materializationをboundedにする。payload付きdirectory entryを拒否し、binary/audio本文とstructured scan後のraw containerはtext再scanしない。merged #65/#68/#70/#71 bytesを保持し、Python 55/55・current/history scanはPASS。新しいproduct ExecPlanは選択せず、exact hosted CI／main read-back前はsource/static candidate扱いとする。
