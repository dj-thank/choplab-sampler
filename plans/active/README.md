...
**独立したreverse PAD末尾修正:** exact-base `main@a0b356c`起点のproduct `8a2a736` / tree `e04dd18`で、逆one-shot/gateのcount/renderだけをrealtime cursor advanceへ統一する。48→60 kHzの旧末尾無音（80→79）と8→48 kHzの1/12-step丸め（12）を固定し、allocation前count＋resetでtrim-copyを回避。LOOP/Windows forceLoopはmain同一のdirect position・4-frame有限render境界を保持し、Desktop playheadを変えない。arbitrary-pitch loop位相は別horizon。hosted JVM/Windows CIとexact reviewがmerge gate。
