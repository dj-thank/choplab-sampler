...
**進行中（単一product convergence / Android音質candidate）:** `h13-github-release-20260902.md`。PR #84はstable Compose 1.11.1 pinをhead `51238b1`からmerge commit `f6cbfdc`へ通常統合し、merged-main 4/4 workflowsもPASS。current `7e7c7ae` / tree `b45b0d7`はAndroidの常時非線形masterをdefault gain 0.9まで透明化し、過負荷を0.98未満へ制限、decoder encoding drift・non-finite・DC-created clippingをfail closed化し、whole-signal DC confirmationとdense limiter invariantを追加した。shared 87×2、Android 288、JVM core 88、lint/APK/AndroidTest compileの99 tasksが全実行PASS。次はこのcandidateのPR/review/CI/merge、PR #69、release hardening、UI brush-up、final-main全gate、immutable `v0.17.1` publication/read-back。旧`v0.17.0`、dirty canonical checkout、既存tag/Releaseは変更しない。
