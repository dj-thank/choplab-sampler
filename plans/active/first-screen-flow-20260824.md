...
- [x] 2026-08-25 00:10:38 JST — Predecessor head `17a959a` received an exact-head clean Codex verdict and passed Windows `32741252127`, iOS `32741252051` and supply-chain `32741252093`. Android `32741252150` passed every pre-device phase, then instrumentation exposed a fixture-only normal-font error: `setGateDeck` called `performScrollTo()` for a demo node whose normal layout intentionally has no scroll ancestor. The fixture now requests scroll only at font scale 1.2+, preserving the large-text route and leaving normal production layout untouched.
- [x] 2026-08-25 00:10:38 JST — Product `3b5dd59` / tree `4985a8c`, with parents prior exact docs head `17a959a` and exact `main@4f56a69`, integrates PR #75 without dropping any gesture/voice fix. Four #75 recorder/test/session blobs are exact; merged ViewModel blob `3c28903` combines daemon microphone start, `stopAsync` and exact STARTING confirmation with engine-owned GATE tokens. GitHub recorded the product 11 seconds before the documentation observation. Python 39/39, exact-head public-surface 399, manifest plus five resource XML parses and `git diff --check` pass; fresh exact-head review and all four hosted workflows remain required.
...
- [x] Resolve all genuinely fixed review threads after latest-head verification; the prior 30/30 are resolved; exact-head review comment `3844915803` found only the stale public-surface count, and its docs-only correction brings the final total to 31/31 resolved.
...
- Review-repair source: reachable two-parent commit `3b5dd59b9271ab9a8a4abce9f03b80301b753861`, tree `4985a8ce8b30c4554e86c1695d2349d0b076024a`, advancing prior PR docs head `17a959a92fb3a08edf83662c8440d16c2de70178` and exact `main@4f56a693b86540d1be13a6eb2153a3cf96ef9396`; the final follow-up only binds documentation. Static policy 39/39, exact-head public-surface 399, six XML parses, exact #71/#73/#75 blobs, exact #74 renderer/test/adapter-test blobs, retained #74 force-loop behavior, retained #68 terminal behavior/exact test, exact #65 transport/test and all BEAT/CHOP/Desktop pointer/ownership regressions pass source/static checks. Predecessor Android run `32741252150` supplied RED evidence only for the corrected normal-font fixture scroll call; hosted compile/runtime checks for the current product are pending and the out-of-ancestry 43d8ace report supplies no current proof.
