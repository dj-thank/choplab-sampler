...
- Post-merge corrective policy suite: 205 tests, failure/error 0; one local skip only because this Windows host lacks symlink creation privilege. Focused scanner is 129. Current and reachable-history scans both pass across 483 candidates with the expanded boundaries; the exact generated debug APK also passes after the stricter PKCS#7 schema check.
