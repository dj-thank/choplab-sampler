...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Windows録音開始中のSTOP | 🧪 source/static / hosted CI待ち | product `c8caf5d` / tree `b09f94e`（exact `main@dfcd9d8`）で、blocking `TargetDataLine.start()`中のSTOPをlatched cancellationとして保持し、復帰後のline/worker公開とcapture readを阻止する。latch regressionはopen/start/close各1回、read 0、一時WAV削除、idle stateを固定。実microphone/device timingと音質は未確認 |
