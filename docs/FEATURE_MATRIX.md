...
| Spotify OAuth / Connect UX | 🧪 local | Native `連携` menuのSpotify Connect panelからAuthorization Code with PKCE、dynamic-port `127.0.0.1` callback、memory-only token、明示的な未設定／認証中／接続済み／エラー状態、認証cancel・late callback無効化、現在再生・保存済みライブラリ・pause/resume・403/404/429等の復帰案内を接続。Spotify Contentはメタデータ／再生制御のみ。実アカウント/provider検証は未実施 |
