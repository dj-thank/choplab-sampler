...
| Versioned stereo-capable project domain | 🧪 foundation | Immutable stereo-capable domain is host-tested。MVP archiveは現在のmono engine状態をversion 1で保存 |
...
| Project save/load | ✅ MVP | `.choplab`手動保存/読込、共有PCM、path traversal/過大manifest/truncated PCM拒否 |
| Autosave/recovery | ✅ MVP | 900ms debounce、fsync後の二世代置換、最新破損時は前世代へ復旧 |
