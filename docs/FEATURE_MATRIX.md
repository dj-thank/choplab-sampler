...
| Android realtime PAD terminal retirement | 🧪 merged source / current PR CI待ち | original product `5dd3d66` / tree `c6a7e9b`はPR #68でmainへ統合済み。PR #58 product `79c2a8b` / tree `ba39013`もexact source/testを保持し、`main@3072eedd`の#65 transport readinessと#70 import境界も統合する。pooled PAD voiceの返却sampleをmixしてからfinished slotをdeactivateし、既存parity fixtureで403 frames、terminal PCM `-61`、retire後inactiveを固定。Python 39/39・public 394・diff PASS、PR #58 exact-head hosted Android gate待ち |
...
| Swing | ✅ | 50–75% |
| Windows transportの先頭step | 🧪 merged source / current PR CI待ち | `transportPlaying/currentStep`をworker開始前のreadiness barrierで公開し、通常開始とScratch復帰でstep 0破棄レースを共通排除。barrier順序、単一step-0 PADのexact-once hit、worker-start失敗時のrecord-arm復元をhost regression化。PR #65は旧exact headの全CIと再review PASS後に`main@3072eedd`へ統合済みで、PR #58 product `79c2a8b`は4つのruntime/test blobをexactに保持。current exact-head hosted再実行待ち |
