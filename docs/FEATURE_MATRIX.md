...
| 固定操作面とlarge-text例外 | 🧪 review repair / prior hosted head | 通常portrait/performanceは固定、normal landscapeはresponsive。font scale 1.2+はheaderを簡素化し4工程を2×2。CAPTUREはlarge textまたはcompact landscape、CHOPとBEAT quick/detailはlarge textで局所scrollし、global chromeは固定。scroll内CHOP／ONE SHOTは完了tap、空CHOPのstationary holdもpointer-upでcapture。GATEは120 ms判定前のinitiating-pointer移動・PAD外離脱を無音cancelし、activation後はそのpointerのupまでmovementをconsumeする。large-text deferred touch、normal touch、stopped-source CHOP、Desktop keyboardのpaired releaseはすべてtoken付き。CHOPのassigned GATEはcapture seamを使わずownership callbackへ進む。Androidはbounded mailbox受理後だけtokenを発行し、touchとsequencerのvoiceへ同じlock-free per-PAD sequenceを付け、releaseはexact voiceだけをfadeする。DesktopはJava Sound start成功後にtokenを返してactive voiceへ保持し、古いpointer/key releaseも新しいvoiceを残したまま自voiceをexact closeする。quick releaseは実triggerから80 ms以上、movement後はTRIM抑止、secondary pointerはrelease/long-pressを継承しない。normal-text BEAT/CHOP実pointer、mailbox拒否、sequencer共存、keyboard token、Desktop trigger失敗／stale releaseを回帰で固定。exact `main@029500a`はproduct `7c57fb4` / tree `085c809`の第二親。prior head `b79f49d`は4 workflows PASSだがcurrent ownership/latest-main deltaはfresh hosted gate待ち。物理touch/TalkBack/Humanは未確認 |
