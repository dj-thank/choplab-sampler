...
| リアルタイム音声安全性 | 🧪 local | 操作queueを512件へ制限し1 block最大64件、Stop Allを容量外で優先しtransportも同じ境界で停止。clear世代境界と128固定latest-wins PAD mailbox、事前確保Voiceを維持。stereo callbackはoutput arrayと3つのmutable frameをloop前に確保し、release bytecodeのsteady sample loopは`new` 0、monitor 0、I/O 0。tone expはcontrol boundaryだけ、soft limiterは非有限値を0へ安全化 |
