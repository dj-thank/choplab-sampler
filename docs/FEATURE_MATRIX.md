...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Reverse ONE_SHOT/GATE resampling tail | ✅ main / exact hosted head | PR #74 exact head `556cd0f`はclean review・threads 0・全4 CI成功後に`main@029500a`へsquash merge。product `1b8b424`で逆one-shot/gateのcount/renderだけをrealtime cursor順序へ統一し、48→60 kHzの旧末尾無音（80→79）と8→48 kHzの1/12-step丸め（12）を固定。2-frame / pitch −5のLOOP/Windows forceLoopは従来3-element境界を保持。実機listening/音質は未確認 |
| Windows録音開始中のSTOP | ✅ main / hosted CI | PR #73を`main@a0b356c`へ統合。blocking `TargetDataLine.start()`中のSTOPがpending lineをclaim/closeしてnative startを解除し、復帰後のworker公開とcapture readを阻止する。latch regressionはcloseによるgate解除、open/start/close各1回、read 0、一時WAV削除、idle stateを固定。exact headはclean review・threads 0・4 workflows PASS。実microphone/device timingと音質は未確認 |
...
| 公開面の資格情報・音源境界 | 🧪 latest-main policy / hosted CI待ち | product `92a2899` / tree `337b9f7`（`main@029500a`統合）で、bounded EOCD/central/local/data-descriptor parserが完全・連続なrecord所有と両metadata copyを検証。signed/signatureless descriptorをCRC/size＋構造境界で判別し、独立raw decoderがstored/deflate/BZIP2/LZMAの完全入力消費とexact出力size/CRCを要求してtrailing data・forged短縮出力を拒否する。current/reachable-history ZIPと追跡しないsymlink targetを検査し、全materialization capを適用。Python 68/68・current/history 396 PASS、exact hosted CI待ち |
