...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Windows録音開始中のSTOP | ✅ main / exact hosted head | blocking `TargetDataLine.start()`中のSTOPがpending lineをclaim/closeしてnative startを解除し、復帰後のworker公開とcapture readを阻止する。close駆動regressionはopen/start/close各1回、read 0、一時WAV削除、idle stateを固定。PR #73 exact head `a1cc5a7`はclean review・全4 CI成功後に`main@a0b356c`へsquash merge。実microphone/device timingと音質は未確認 |
| Android / Windows取込名・autosave境界 | ✅ main / exact hosted head | archive-compatible名規則をsharedへ集約し、Androidのprovider/URI/final fallback契約を保持したままDesktop `File.name`もPCM公開前に非空・最大240 UTF-16単位へ正規化。切断後blank再評価、surrogate pair分断防止、shared common contract、両platformのarchive round trip回帰を含む。PR #71 exact head `f7ebd01`はclean review・全4 CI成功後に`main@dfcd9d8`へsquash merge。実provider/filesystem autosaveは未確認 |
...
| 公開面の資格情報・音源境界 | 🧪 latest-main policy / hosted CI待ち | `scripts/check_public_surface.py`が認証情報、署名素材、音源候補をfail-closedで検査。bounded EOCD preflightで`ZipFile`生成前にcentral-directory bytesと実record数を検証し、forged countを含め4096-entry capを適用。local filename/flags/compression/non-descriptor CRC・sizeのcentral一致をsuffix/payload判断前に要求し、unclaimed prefix/trailing bytesを拒否。current treeとreachable historyのZIP member、archive comment、central filename/comment/extra、local-header filename/extraを非展開・byte-preservingに検査し、NUL-safe per-parent raw path/blob履歴からnewline path、ZIP alias、merge-resultを列挙してtree/symlink/gitlinkをblob capへ含めない。512 KiB/member、4 MiB/archive、100:1圧縮率に加え履歴blob数／container合計を制限。product `7e98d9c` / tree `4d43ec3`、merged `main@a0b356c`保持、Python 61/61・current/history 395 scan PASS、exact hosted CI待ち。 |
...
| Swing | ✅ | 50–75% |
| Windows transportの先頭step | ✅ main / exact hosted head | `transportPlaying/currentStep`をworker開始前のreadiness barrierで公開し、通常開始とScratch復帰でstep 0破棄レースを共通排除。barrier順序、単一step-0 PADのexact-once hit、worker-start失敗時のrecord-arm復元をhost regression化。PR #65 exact head `4c8f2c6`はclean reviewと全4 CI成功後、`main@3072eed`へexpected-head squash merge。物理Windows音声／実scheduler latencyは未確認 |
