...
| 波形viewportのアクセシビリティ | 🧪 framework-node/device focus-path | autosave非依存の固定PCM fixtureで2本指pinch/pan、前/次/reset semantics、overview geometry、S/E/chopの48dp幅・高さ・端点clamp・可逆nudgeを検証。Compose ATFとAndroid framework node経路でS/E/chop1-5のdepth-first tree順、focus action公開、custom actionによるstate更新をAVD/Pixelで確認。物理Pixelの実TalkBack serviceで旧遮蔽領域へのfocus ring移動も確認。TTS内容・完全な読み上げ順・service custom-action menuはHuman境界として分離 |
