...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Windows EXE版のPAD操作 | 🧪 current local / historical CI / Release | `:desktop` が `:shared` の Android-origin deck UI/model と `:jvm-core` のproject/archive/export処理を使用。4工程、波形、4×4 PAD、PAD editor、BANK A–D、16-step transport、KEY/TONE/LEVEL/REVERSE/GATE/LOOP/CHOKE、scratch、ローカルWAV、マイク、driver loopback候補、Undo/Redo、`.choplab`保存/復元/autosave、4小節WAV書き出しを接続。current local candidateではsource録音停止後にCHOPへ進み、VOICE開始時に選択Beat loopを先頭から再始動し、startup project sessionでもautosaveを継続。実ドライバー音質・レイテンシは未確認 |
...
| Spotify OAuth / Connect UX | ✅ local / device instrumentation | Native `連携` menuのSpotify Connect panelからAuthorization Code with PKCE、dynamic-port `127.0.0.1` callback、memory-only token、明示的な未設定／認証中／接続済み／エラー状態、認証cancel・late callback無効化、現在再生・空／異常を区別する保存済みライブラリ、pause/resume、拒否・既定browser・loopback port・network・401/403/404/429等の復帰案内を接続。Spotify Contentはメタデータ／再生制御のみ。実アカウント/provider、物理音声品質、screen-reader speechは未実施 |
