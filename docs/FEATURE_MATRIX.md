...
| Android realtime PAD terminal retirement | 🧪 merged source / current PR CI待ち | original product `5dd3d66` / tree `c6a7e9b`はPR #68でmainへ統合済み。PR #58 product `986ea4b` / tree `6b28819`もexact source/testを保持し、`main@dfcd9d8`の#65 transport readinessと#71 shared import-name境界も統合する。pooled PAD voiceの返却sampleをmixしてからfinished slotをdeactivateし、既存parity fixtureで403 frames、terminal PCM `-61`、retire後inactiveを固定。Python 39/39・public 395・diff PASS、PR #58 exact-head hosted Android gate待ち |
...
| 固定操作面とlarge-text例外 | 🧪 review repair / prior emulator | 通常portrait/performanceは固定、normal landscapeはresponsive。font scale 1.2+はheaderを簡素化し4工程を2×2。CAPTUREはlarge textまたはcompact landscape、CHOPとBEAT quick/detailはlarge textで局所scrollし、global chromeは固定。scroll内CHOP／ONE SHOTは完了tap、空CHOPのstationary holdもpointer-upでcapture。GATEは120 ms判定後にmovementをconsumeして物理pointer-upまで保持し、activation直後を含むquick releaseも実triggerから80 ms preview、古いtimerはretriggerをreleaseせず、movement後はTRIMを抑止する。PAD起点のactivation前swipeはcontroller action 0、large-text波形起点vertical swipeはCHOP親scrollへ到達し、非scroll波形のvertical dragはtapを発火しない。mode変更時はpointer handlerを再起動し、非scroll PADはpress-down発音を維持。48dp cellで隠れるLOOP/DRM/VOXはsemanticsへ常時保持する。PRs #62/#65/#66/#67/#68/#70/#71を含む`main@dfcd9d8`へreachable product `986ea4b` / tree `6b28819`で統合済み。物理touch/TalkBack/Humanは未確認 |
...
| Swing | ✅ | 50–75% |
| Windows transportの先頭step | 🧪 merged source / current PR CI待ち | `transportPlaying/currentStep`をworker開始前のreadiness barrierで公開し、通常開始とScratch復帰でstep 0破棄レースを共通排除。barrier順序、単一step-0 PADのexact-once hit、worker-start失敗時のrecord-arm復元をhost regression化。PR #65は旧exact headの全CIと再review PASS後にmainへ統合済みで、PR #58 product `986ea4b`は`main@dfcd9d8`上で4つのruntime/test blobをexactに保持。current exact-head hosted再実行待ち |
