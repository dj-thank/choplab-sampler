...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Android取込名 / autosave境界 | 🧪 merged source / current PR CI待ち | original product `b7364ee` / tree `dae6252`はPR #70で`main@33308814`へ統合済み。PR #58 product `c2e3aee` / tree `ec8702b`もexact source/testを保持する。provider `DISPLAY_NAME`、URI fallback、最終`sample`をproject/archiveと同じ非空・最大240 UTF-16単位へdecode公開前に正規化し、切断後が空白だけならfallbackを再評価、surrogate pairも分断しない。実provider/device autosaveは未確認 |
...
| Android realtime PAD terminal retirement | 🧪 merged source / current PR CI待ち | original product `5dd3d66` / tree `c6a7e9b`はPR #68でmainへ統合済み。PR #58 product `c2e3aee` / tree `ec8702b`もexact source/testを保持し、`main@33308814`の#70 import境界も統合する。pooled PAD voiceの返却sampleをmixしてからfinished slotをdeactivateし、既存parity fixtureで403 frames、terminal PCM `-61`、retire後inactiveを固定。Python 39/39・public 394・diff PASS、PR #58 exact-head hosted Android gate待ち |
