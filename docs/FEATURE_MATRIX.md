...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Reverse PAD resampling tail | 🧪 source/static / hosted CI待ち | product `6151317` / tree `73d6f68`（exact-base `main@a0b356c`）で、逆再生のcount/renderをrealtimeと同じone-shot cursorのadvance順序へ統一。48→60 kHz、64 source framesでは旧80 frames＋範囲外末尾無音1 frameに対し、同じ79 frames、最後の有効PCM非0を固定。cursorはallocation前にcountしてresetするためtrim-copy不要。Android callback、project schema、forward経路は不変 |
