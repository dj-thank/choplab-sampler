...
| Autosave/recovery | ✅ historical device / current source candidate | 900ms debounce、SHA-256に結合した世代revision、store再生成後も古い/equal revisionを拒否、最新の検証済み世代を選ぶpending復旧、三世代保持。Desktop closeはstartup recoveryを待ち、admitted project publicationをinvalidate後にdrainしてからstate＋ProductionSession revisionを取得し、live audioを停止してからclose-time autosaveを待つ。scheduled saveは一度だけ即時flush、successful running bodyは重複送信せず待機、failed latest bodyはcloseで一度retryし、新revisionは旧成功work後に一度だけ追随保存する。回帰はstartup recoveryのBPM 127保持、same-revision exact-one、公開前`loadWav`競合、newer async revisionの二世代順序、transient failure retry、stop-before-waitを固定。archiveが意図的に保存しないlaunch routingは永続化oracleにしない。process restart後の次editも保護。新Desktop testのhosted実行と現候補のDEVICE再実行は未実施。手動上書き前にもアプリ内安全コピーを作成 |
