...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Androidマイク録音開始中のSTOP | 🧪 exact-main source / hosted CI待ち | product `8ffc1f4` / tree `4eee53d`（`main@a0b356c`）で、blocking `AudioRecord.startRecording()`をlifecycle lock外へ移し、STOPがpending inputをclaimして`stop`/`release`できるよう修正。exact-identity cleanupとgeneration間guardでlate worker、二重release、取消中のreplacement startを阻止。latch regressionはstop/release各1、read 0、一時WAV削除を固定。実microphone/device timingと音質は未確認 |
