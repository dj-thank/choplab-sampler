...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Windows録音開始中のSTOP | 🧪 merged source / current PR CI待ち | PR #73はblocking `TargetDataLine.start()`中のSTOPがpending lineをclaim/closeしてnative startを解除し、復帰後のworker公開とcapture readを阻止する修正を`main@a0b356c`へ統合済み。latch regressionはcloseによるgate解除、open/start/close各1回、read 0、一時WAV削除、idle stateを固定。PR #58 product `643546b` / tree `12d8f26`はexact source/testを保持し、実microphone/device timingと音質は未確認 |
| Android / Windows取込名・autosave境界 | 🧪 merged source / current PR CI待ち | PR #71はarchive-compatible名規則をsharedへ集約して`main@dfcd9d8`へ統合済み。PR #58 product `643546b` / tree `12d8f26`はその六つのshared/Android/Desktop product-test blobをexact保持し、exact `main@a0b356c`を統合する。Androidのprovider/URI/final fallback契約を維持し、Desktop `File.name`もPCM公開前に非空・最大240 UTF-16単位へ正規化。切断後blank再評価、surrogate pair分断防止、shared common contract、両platformのarchive round trip回帰を含む。実provider/filesystem autosaveは未確認 |
...
| Android realtime PAD terminal retirement | 🧪 merged source / current PR CI待ち | original product `5dd3d66` / tree `c6a7e9b`はPR #68でmainへ統合済み。PR #58 product `643546b` / tree `12d8f26`もexact source/testを保持し、`main@a0b356c`の#65 transport readiness、#71 shared import-name境界、#73 recorder cancellationを統合する。pooled PAD voiceの返却sampleをmixしてからfinished slotをdeactivateし、既存parity fixtureで403 frames、terminal PCM `-61`、retire後inactiveを固定。Python 39/39・public 396・diff PASS、PR #58 exact-head hosted Android gate待ち |
...
| 固定操作面とlarge-text例外 | 🧪 review repair / prior emulator | 通常portrait/performanceは固定、normal landscapeはresponsive。font scale 1.2+はheaderを簡素化し4工程を2×2。CAPTUREはlarge textまたはcompact landscape、CHOPとBEAT quick/detailはlarge textで局所scrollし、global chromeは固定。scroll内CHOP／ONE SHOTは完了tap、空CHOPのstationary holdもpointer-upでcapture。GATEは120 ms判定前のinitiating-pointer移動・PAD外離脱を無音cancelし、activation後はそのpointerのupまでmovementをconsumeする。controller発行のpad-wide tokenがtouch/keyboard/programmatic retriggerを横断して古いtimerを拒否し、quick releaseは実triggerから80 ms以上、movement後はTRIM抑止、secondary pointerはrelease/long-pressを継承しない。PAD起点のactivation前swipeはcontroller action 0、large-text波形起点vertical swipeはCHOP親scrollへ到達し、非scroll波形のvertical dragはtapを発火しない。horizontal/pinchに加えてrotation-only二指gestureもclaimしてtapをcancelする。mode変更時はpointer handlerを再起動し、非scroll PADはpress-down発音を維持。48dp cellで隠れるLOOP/DRM/VOXはsemanticsへ常時保持する。PRs #62/#65/#66/#67/#68/#70/#71/#73を含むexact `main@a0b356c`はproduct `643546b` / tree `12d8f26`のancestryへ統合済み。prior Android headは25 testsを実行し、canceled gesture後のsingle-tap callbackをdouble-tap window前にassertした2 fixtureだけ失敗。current productはreal tap後にcontrolled clockを400 ms進める。物理touch/TalkBack/Humanは未確認 |
