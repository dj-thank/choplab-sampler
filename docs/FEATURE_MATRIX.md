...
| Android realtime PAD terminal retirement | 🧪 merged source / current PR CI待ち | original product `5dd3d66` / tree `c6a7e9b`はPR #68で`main@2786c372`へ統合済み。PR #58 product `ba473e0` / tree `814e037`もexact source/testを保持する。pooled PAD voiceの返却sampleをmixしてからfinished slotをdeactivateし、既存parity fixtureで403 frames、terminal PCM `-61`、retire後inactiveを固定。callback追加は既存Voice＋primitiveだけでallocation/lock/I/Oなし。Python 39/39・public 394・diff PASS、PR #58 exact-head hosted Android gate待ち |
...
| 固定操作面とlarge-text例外 | 🧪 review repair / prior emulator | 通常portrait/performanceは固定、normal landscapeはresponsive。font scale 1.2+はheaderを簡素化し4工程を2×2。CAPTUREはlarge textまたはcompact landscape、CHOPとBEAT quick/detailはlarge textで局所scrollし、global chromeは固定。scroll内CHOP／ONE SHOTは完了tap、空CHOPのstationary holdもpointer-upでcapture。GATEは120 ms判定後にmovementをconsumeして物理pointer-upまで保持し、短tapは80 ms preview、古いpreview timerは新しいretriggerをreleaseせず、mode変更時はpointer handlerを再起動する。PAD起点のactivation前swipeはcontroller action 0、波形起点の1本指vertical swipeはCHOP親scrollへ到達し、非scroll PADはpress-down発音を維持。48dp cellで隠れるLOOP/DRM/VOXはplayMode/contentKind semanticsへ常時保持し、status stripがないcompact landscapeではheader内にfeedbackを残す。PRs #62/#66/#67/#68を含む`main@2786c372`へreachable product `ba473e0` / tree `814e037`で統合済み。物理touch/TalkBack/Humanは未確認 |
