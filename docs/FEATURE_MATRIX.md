...
| 固定操作面とlarge-text例外 | 🧪 review repair / prior emulator | 通常portrait/performanceは固定、normal landscapeはresponsive。font scale 1.2+はheaderを簡素化し4工程を2×2。CAPTUREはlarge textまたはcompact landscape、CHOPとBEAT quick/detailはlarge textで局所scrollし、global chromeは固定。scroll内CHOP／ONE SHOTは完了tap、空CHOPのstationary holdもpointer-upでcapture。GATEは120 ms判定後からpointer-upまたは発火後node cancellationまでexact-once releaseで保持し、短tapは80 ms preview、mode変更時はpointer handlerを再起動する。PAD起点のactivation前swipeはcontroller action 0、非scroll PADはpress-down発音を維持。48dp cellで隠れるLOOP/DRM/VOXはplayMode/contentKind semanticsへ常時保持し、status stripがないcompact landscapeではheader内にfeedbackを残す。PR #62は`main@6b645ca`へ統合済みで、追加CHOP／short-preview repairはreachable product `5484dbd` / tree `ea54509`で同じmainへ統合済み。物理touch/TalkBack/Humanは未確認 |
