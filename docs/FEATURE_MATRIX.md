...
| PAD付き | ✅ | 4 BANK × 16 PAD |
| 「おとひろい」正式UI | 🧪 source | 固定 `CHOP / PAD / SEQ / SOURCE` コンソールへ刷新。ローカルbuild/lint済み、今回版の実機visual確認待ち |
| スクロールなし操作 | 🧪 source | portrait/landscapeをconstraint-driven配置。UI sourceにscroll APIなし、今回版の端末clipping確認待ち |
...
| ハードウェア系サンプラーの操作感 | 🧪 source | 4モード、固定PAD、触覚、二段階clearを独自UIで再構成。今回版の実機操作確認待ち |
| Pitch | ✅ | ±24 st、速度も連動 |
| Reverse | ✅ | PAD別 |
| One Shot / Gate | ✅ | PAD別 |
| Choke group | ✅ | 1–4 |
