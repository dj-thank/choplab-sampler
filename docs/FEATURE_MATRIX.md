...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Reverse PAD resampling tail | 🧪 source/static / hosted CI待ち | product `830e12a` / tree `6d9f921`（exact-base `main@a0b356c`）で、逆one-shot/gateのcount/renderをrealtime cursor順序へ統一し、LOOPはfull-range cycleを保持。48→60 kHzの旧末尾無音（80→79 frames）、8→48 kHzの1/12-step丸め（12 frames）、2-frame逆LOOP（one-shot 3 / loop 4）を固定。allocation前count＋resetでtrim-copy不要。Android callback、schema、forward経路は不変 |
