...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Reverse PAD resampling tail | 🧪 source/static / hosted CI待ち | product `6bb8045` / tree `f2f9912`（exact-base `main@a0b356c`）で、逆再生をexclusive endではなく最後のincluded frameから数え、範囲内positionだけをrenderする。48→60 kHz、64 source framesでは旧80 frames＋範囲外末尾無音1 frameに対し、realtime cursorと同じ79 frames、最後の有効PCM非0を固定。Android callback、project schema、forward経路は不変 |
