...
| 波形viewportのアクセシビリティ | 🧪 local / historical framework-node / device focus-path | autosave非依存の固定PCM fixtureで2本指pinch/pan、前/次/reset、overview、S/E/chopの48dp・端点・可逆nudgeを検証。物理Pixelの実TalkBack focus ringは既存receiptあり。現候補のアクセシビリティclickはLOCAL build済みだが、専用AVDのGoogle Play image欠落によりframework-node再実行はBLOCKED。TTS内容・完全な読み上げ順・service custom-action menuはHUMAN_GOへ分離。 |
...
| Project save/load | ✅ MVP/local | `.choplab`手動保存/読込、共有PCM16 WAV、schema 1–5 migration、path traversal/過大manifest/malformed WAV/進捗0 InputStreamをfail-closedで拒否 |
