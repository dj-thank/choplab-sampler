...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Android / Windows取込名・autosave境界 | 🧪 merged-main source / prior hosted head | `main@dfcd9d8`でarchive-compatible名規則をsharedへ集約。Androidのprovider/URI/final fallback契約を保持し、Desktop `File.name`もPCM公開前に非空・最大240 UTF-16単位へ正規化する。切断後blank再評価、surrogate pair分断防止、shared common contract、両platformのarchive round trip回帰を含む。#71旧exact headは全CIとclean reviewがPASSし、現在の#64統合headでhosted再実行待ち。実provider/filesystem autosaveは未確認 |
...
| Swing | ✅ | 50–75% |
| Windows transportの先頭step | 🧪 merged-main source / prior hosted head | `transportPlaying/currentStep`をworker開始前のreadiness barrierで公開し、通常開始とScratch復帰でstep 0破棄レースを共通排除。barrier順序、単一step-0 PADのexact-once hit、worker-start失敗時のrecord-arm復元をhost regression化。PR #65旧exact headは全CIと再reviewがPASSし、`main@3072eed`へmerge済み。現在の#64統合headでhosted再実行待ち |
...
| Autosave/recovery | ✅ historical device / current source candidate | 900ms debounce、SHA-256に結合した世代revision、store再生成後も古い/equal revisionを拒否、最新の検証済み世代を選ぶpending復旧、三世代保持。Desktop closeはstartup state公開を待ち、close claim後の復元音声hydrateをskipし、admitted project publicationをinvalidate後にdrainしてからstate＋ProductionSession revisionを取得する。復元hydrateはstate公開前に取得したoperation ownershipを再利用するため、公開後に始まったuser WAV/project操作をsupersedeしない。出力device失敗は復元sourceを保持して通常WAV loadと同じactionable errorを公開する。復元失敗の空placeholderは後続editがない限り保存しない。live audio停止後にclose-time autosaveを待ち、scheduled saveは一度だけ即時flush、successful running bodyは重複送信せず待機、failed latest bodyはcloseで一度retryし、新revisionは旧成功work後に一度だけ追随保存する。回帰はstartup recoveryのBPM 127保持／hydrate 0回、corrupt recovery後のoriginal bytes保持、post-publication user load優先、device failure可視化、same-revision exact-one、公開前`loadWav`競合、newer async revisionの二世代順序、transient failure retry、stop-before-waitを固定。archiveが意図的に保存しないlaunch routingは永続化oracleにしない。新Desktop testのhosted実行と現候補のDEVICE再実行は未実施。手動上書き前にもアプリ内安全コピーを作成 |
