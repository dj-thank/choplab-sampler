...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Android取込名 / autosave境界 | 🧪 source / hosted CI待ち | provider `DISPLAY_NAME`、URI fallback、最終`sample`をdecode公開前にproject/archiveと同じ非空・最大240 UTF-16単位へ正規化。上限位置のsurrogate pairを分断せず、exact archive round tripをunit regressionへ固定。音声bytes/schema/provider I/Oは不変。現sandboxではGradle distributionを利用できず、実provider/device autosaveは未確認 |
