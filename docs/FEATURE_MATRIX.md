...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Pattern/master full-bar parity | 🧪 current local | `c2dd5c7`でsingle PAD / step 0 / one barのrealtime Voice+master expectedとoffline WAVを全frame比較。REDはframe 402でoffline末尾sample欠落61、mix-before-retireの1行修正後max delta≤1、last energetic frame一致。Android 229 / full 152-task gate PASS。polyphony/stereo/physical listeningは別 |
