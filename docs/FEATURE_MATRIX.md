...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Reverse ONE_SHOT/GATE resampling tail | 🧪 source/static / hosted CI待ち | product `1b8b424` / tree `614c93f`（exact-base `main@a0b356c`）で、逆one-shot/gateのcount/renderだけをrealtime cursor順序へ統一。48→60 kHzの旧末尾無音（80→79）と8→48 kHzの1/12-step丸め（12）を固定し、allocation前count＋resetでtrim-copy不要。review報告の2-frame / pitch −5でLOOP/Windows forceLoopはmain同一のdirect position・3-element有限render境界、Desktop playheadは既存の単調mappingを保持。Android callback、schema、forward経路は不変 |
