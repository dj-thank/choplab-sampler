...
| Android realtime PAD terminal retirement | 🧪 merged source / current PR CI待ち | original product `5dd3d66` / tree `c6a7e9b`はPR #68でmainへ統合済み。PR #58 product `74f2cee` / tree `385cafb`もterminal-sample-before-retirement処理とexact parity testを保持し、runtimeへper-voice GATE ownershipだけを追加する。403 frames、terminal PCM `-61`、retire後inactiveの既存oracleは不変。Python 39/39・public 398・diff PASS、PR #58 exact-head hosted Android gate待ち |
...
| 固定操作面とlarge-text例外 | 🧪 review repair / prior hosted head | 通常portrait/performanceは固定、normal landscapeはresponsive。font scale 1.2+はheaderを簡素化し4工程を2×2。CAPTUREはlarge textまたはcompact landscape、CHOPとBEAT quick/detailはlarge textで局所scrollし、global chromeは固定。scroll内CHOP／ONE SHOTは完了tap、空CHOPのstationary holdもpointer-upでcapture。GATEは120 ms判定前のinitiating-pointer移動・PAD外離脱を無音cancelし、activation後はそのpointerのupまでmovementをconsumeする。large-text deferred touch、normal touch、Desktop keyboardのpaired releaseはすべてtoken付き。Androidはbounded mailbox受理後だけtokenを発行し、touchとsequencerのvoiceへ同じlock-free per-PAD sequenceを付け、releaseはexact voiceだけをfadeする。DesktopはJava Sound trigger成功後だけownershipを公開する。quick releaseは実triggerから80 ms以上、movement後はTRIM抑止、secondary pointerはrelease/long-pressを継承しない。PAD起点のactivation前swipeはcontroller action 0、large-text波形起点vertical swipeはCHOP親scrollへ到達し、非scroll波形のvertical dragはtapを発火しない。horizontal/pinch/rotation-only二指gestureはtapをcancelする。normal-text実pointer、mailbox拒否、sequencer共存、keyboard token、Desktop trigger失敗を新規回帰で固定。exact `main@a0b356c`はproduct `74f2cee` / tree `385cafb`の第二親。prior head `b79f49d`は4 workflows PASSだがcurrent ownership deltaはfresh hosted gate待ち。物理touch/TalkBack/Humanは未確認 |
