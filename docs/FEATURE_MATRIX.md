...
| 公開面の資格情報・音源境界 | 🧪 latest-main policy / hosted CI待ち | `scripts/check_public_surface.py`が認証情報、署名素材、音源候補をfail-closedで検査。bounded EOCD/central preflightとlocal record/data-descriptor parserがbyte 0からcentral directoryまで完全・連続な所有範囲、両metadata copy、圧縮stream完全消費を検証し、forged count、interior gap、trailing compressed data、compressed-only directory payloadを拒否。current treeとreachable historyのZIP metadata/memberを非展開・byte-preservingに検査し、現行symlink targetは追跡せず`readlink`結果を検査。NUL-safe per-parent raw path/blob履歴はnewline path、ZIP alias、merge-resultを列挙し、tree/symlink/gitlinkをblob capへ含めない。entry/member/archive/圧縮率/履歴materializationを制限。product `53a2e3f` / tree `3c90b33`、merged `main@a0b356c`保持、Python 65/65・current/history 395 scan PASS、exact hosted CI待ち。 |
