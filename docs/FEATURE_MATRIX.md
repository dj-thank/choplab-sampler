...
| 固定操作面とlarge-text例外 | ✅ current local / emulator | 通常portrait/performanceは固定、normal landscapeはresponsive。CAPTUREはlarge textまたはcompact landscape、BEAT quick/detailはlarge textで局所的にscrollし、global chromeは固定。200%で48dp-safe 4×4 PAD、完全なB01–B16、短縮表示＋完全semantics、物理swipeによる全primary control到達を確認 |
