...
| Autosave/recovery | ✅ historical device / current source candidate | 900ms debounce、SHA-256に結合した世代revision、store再生成後も古い/equal revisionを拒否、最新の検証済み世代を選ぶpending復旧、三世代保持。Desktop closeはstate＋ProductionSession revisionのimmutable snapshotを先に取得しlive audioを停止してから、scheduled saveを一度だけ即時flushするかrunning saveの同じbodyを重複送信せず待つ。close snapshotが新revisionなら旧work後に一度だけ追随保存する。blocked-save回帰はsame-revision exact-one、newer async revisionの二世代順序、stop-before-waitを固定。process restart後の次editも保護。新Desktop testのhosted実行と現候補のDEVICE再実行は未実施。手動上書き前にもアプリ内安全コピーを作成 |
