...
| 要望 | 実装 / 確認層 | 備考 |
|---|---:|---|
| Reverse PAD resampling tail | 🧪 source/static / hosted CI待ち | product `f2d4952` / tree `6e40fe6`（exact-base `main@a0b356c`）で、逆one-shot/gateのcount/renderをrealtime cursor順序へ統一し、LOOP/Windows forceLoopはfull-range cycleを保持。48→60 kHzの旧末尾無音（80→79）、8→48 kHzの1/12-step丸め（12）、2-frame reverse one-shot/LOOP/forceLoop（3/4/4）を固定。allocation前count＋resetでtrim-copy不要。Android callback、schema、forward経路は不変 |
