...
- Current product/test commit: `401098d1691a144c99d7b55b7f6a43b4e0db96ae`
- Current product tree: `fb764719ac3ea4be70e069756d8d2e44fe6511fc`
...
- Focused regressions: save/export during hydration; failed replacement fallback; successful replacement versus stale recovery failure during close; playback after device-load failure; unchanged recovery byte preservation; teardown-exception flush. The first hosted run exposed only an over-specific playback-message oracle; the follow-up requires the exact pre-action device-error status to remain unchanged while playback calls remain zero.
- Local gates: Python policy 39/39, public-surface 396 candidates, conflict-marker scan, and `git diff --check` pass. Gradle 9.7.1 is unavailable locally, so hosted Windows compilation/tests/package and exact-head review are required.
