...
- Exact-head review follow-up: recovery records success/failure independently from autosave work. Close suppresses an unchanged failure placeholder, while a later edit/revision can still own persistence. Recovery claims its project-operation epoch before construction completes and reuses that epoch for post-publication hydration, so a later user load/open is never invalidated by startup work. Hydration runs only while close has not claimed shutdown, and an output-device failure retains the source with the normal WAV-load device error.
- Additional regressions: the blocked BPM 127 recovery includes source PCM and requires zero engine loads after close claims the lifecycle. A corrupt primary archive is restored after the recovery error becomes visible; close must preserve the exact bytes as primary and create no previous generation. An unconfined observer begins a large WAV load at recovered-state publication and requires that user audio to be the sole engine load/current source. A separate failing engine requires the recovered source plus an actionable device-error status.
...
- Superseded-head GREEN / review RED: exact head `c17ab19dd75145d2477119ab508448bd3fecf318` passed Android run `32723877713`, Windows run `32723877725`, iOS run `32723877824` and supply-chain run `32723877733`; subsequent exact-head Codex review found the hydration ownership and device-failure regressions fixed in product `8b6c404`.
- Prior repaired-head GREEN: exact head `ce201141da543540e23b31524ac8e9e0ef0c169d` passed Android run `32726261739`, Windows run `32726261784`, iOS run `32726261818` and supply-chain run `32726261852`. Main then advanced through #71, so this complete hosted receipt remains revision-bound to the pre-integration head.
- Local evidence: Python policy 39/39, public-surface scan 395 candidates, conflict-marker scan and `git diff --check` PASS after integrating exact `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`. The six #71 product/test blobs plus its hardening-plan blob and the two transport-only #65 files match merged main byte-for-byte; #65 controller/test behavior is present beside the recovery regressions. Gradle 9.7.1 cannot create its default-cache lock directory here, so the focused Desktop tests and full Windows workflow require hosted read-back on the published exact head.
...
## Shared Android/Desktop import-name merged-main receipt — 2026-08-24

- Product source: reachable integration `e0d3fa1df5862bcfa038812bb12ecf6d2c45911e`, tree `1558c4a2331e8ef3a7b2809b38f264e671c188a6`, with parents prior exact PR head `d2c99fc2d4bb9c84bf6366a7f2568cca88294422` and merged `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`; PR #71 was subsequently squash-merged as `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`.
- Main preservation: #65 controller/transport/test behavior remains present, with transport-only blobs exact `c92613c` / `073696e`; #68 runtime/tests remain `de686ba` / `9d51556`. #70 Android provider/URI selection and its archive regression are preserved through the shared helper.
...
- Prior exact-head provider evidence: `d2c99fc` passed clean Codex review, Android `32724477061`, Windows `32724477012`, iOS `32724476941` and supply-chain `32724476968`, with review threads 0. These receipts remain bound to that exact head.
- Current integration: product `9bb8eff6d0c43b7648869004eeddec82f93ed9f2` / tree `59e8ef0198ade5d0fa19636745ebc3c52e9f3c42` preserves the six #71 product/test blobs and its hardening-plan blob byte-for-byte while adding only the #64 recovery repairs outside those paths. Python policy 39/39, public-surface 395 candidates, conflict-marker scan and `git diff --check` pass.
- Gate: merged source plus revision-bound prior-head hosted evidence only. Current #64 hosted Android/Windows/shared execution and exact-head review remain required; filesystem/provider imports, recovery, playback, publication and Human evidence remain separate.

## Desktop transport step-zero ordering merged-main receipt — 2026-08-24

- Product source: reachable integration product `08fb123888fb840496d34e4ba7a586013e1305f6`, tree `dc9a9e8563e18168c5d20af9084ffaab01f0f742`, with parents prior exact PR head `5d630c8a769e4b840bba9914f59bc0ec1c705638` and merged `main@333088147cdc77932efc41b90a08eb37e1c1cf42`; squash-merged as `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`.
...
- Current integration: product `99bd386` retains both transport-only files byte-for-byte and reconciles the controller/test hunks with the current recovery fixes. Fresh hosted execution remains bound to the final PR head.
