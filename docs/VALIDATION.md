...
- Product/policy source: reachable policy commit `f3fe80c6933f42622ed3542fa108916a13af5465`, tree `56a3a437b66a2256056754f2b2ee6ee8b4cfaba1`, based on prior exact PR head `83127d75146c5f49113df317bc2ccef360d2f155`, whose integration parent retains merged `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`; this section is in a later documentation-only commit. Merged #65/#68/#70/#71 runtime, tests and documentation are retained.
- Contract: safe-named ZIP member text, archive/member comments and entry extra fields in the current tree and reachable historical ZIP blobs are scanned with the ordinary secret patterns. Replacement decoding preserves ASCII tokens beside malformed bytes. Historical candidates are derived from NUL-delimited per-parent raw path/blob records before blob-ID deduplication and accept only regular-file modes. The scanner rejects more than 4,096 entries before member opening, payload-bearing directory entries, members above 512 KiB, archives above 4 MiB scanned content/metadata and declared compression above 100:1. History is capped at 128 unique ZIP blobs, 16 MiB each and 64 MiB aggregate; exceeding any cap fails the gate.
...
- Tests: focused public-surface policy 20/20 and complete Python policy 55/55 passed. Fixtures additionally cover malformed-byte-adjacent tokens, extra fields, ZIP-named symlinks and merge-result-only archives, alongside the prior alias/tree/comment/control/member/limit/binary boundaries.
