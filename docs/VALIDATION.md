...
- Product source: reachable two-parent commit `5484dbd31e750fc7f7afac1ce9fce010c8b4b235`, tree `ea54509c42fa06c077220b04988aa0d624da7554`; PR #52 is merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`, PR #62 is merged through `main@6b645ca5005f905e93c572edfc1d375d4a6eeeb5`, and that exact main revision is the product commit's main-side parent. The final follow-up commit is a documentation-only anchor update.
...
- Instrumentation isolation: `FirstScreenFlowDeviceTest` renders deterministic in-memory CAPTURE and CHOP states/controller seams and never launches autosave recovery, clears app data or mutates retained projects. Real pointer regressions require PAD-origin CHOP, ONE SHOT and pre-activation GATE swipes to move their parent with zero controller calls; a completed ONE SHOT tap to dispatch ordered actions; a post-composition ONE_SHOT→GATE change to activate before pointer-up and release afterward; a short GATE tap to retain at least 80 ms; GATE-trigger ownership to release exactly once when long-press navigation removes the grid; a held empty CHOP pad to wait through long-press timeout and capture on pointer-up; and a non-scroll PAD to select and trigger before pointer-up. Earlier run `32712301910` stopped in test compilation because `longClick` was unavailable; the integrated source retains #62's proven import while the five new ownership tests use explicit `down` / clock advancement / `up` or `cancel` injection. Later Android run `32716871329` compiled and reached instrumentation, where `largeTextCanReachDemoAndBothBeatSurfacesByScrolling` failed because its 48 dp assertion measured inner `B01` glyph semantics. The integrated fixture now scrolls/asserts that label only for visibility, measures the assigned PAD button semantics, and separately checks its full ONE SHOT/DRM description. These integrated device tests remain source coverage until the new exact head executes.
- Current-container checks: `git diff --check` PASS; Python policy suite 39/39 PASS; public-surface scan 416 candidates PASS; all six Android XML files parse successfully.
