...
- Product/policy source: reachable policy product `53a2e3f6606d26607398ff1cf34d5708591f1d5d`, tree `3c90b3392bdc9eba07f4697040e901d2ed321f71`, advancing prior exact policy head `4c93bf2dfe6f74e1699694ae9cc5bd8a82cb3cd1` on squash-merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; this section is in a later documentation-only commit. Merged #65/#68/#70/#71/#73 runtime, tests and documentation are retained.
- Contract: a bounded end-of-central-directory preflight enforces central-directory byte bounds, parses each central record and compares the actual count before `ZipFile` can allocate `ZipInfo` objects, and rejects multi-disk/ZIP64 sentinel cases. All local records, metadata, declared compressed spans and optional data descriptors must claim one contiguous range from byte zero to the central directory, with descriptor CRC/sizes matching the central copy. Local filenames, flags, compression methods and non-descriptor CRC/sizes must also match before suffix policy or payload bounds are trusted, and decompression must consume the complete declared span without trailing or unconsumed bytes. Safe-named ZIP member text, archive comments, central-directory filenames/comments/extra fields and local-header filenames/extra fields in the current tree and reachable historical ZIP blobs are then scanned with the ordinary secret patterns under one byte budget. Replacement decoding preserves ASCII tokens beside malformed bytes. Historical candidates are derived from NUL-delimited per-parent raw path/blob records before blob-ID deduplication and accept only regular-file modes; current-tree symlink targets are scanned through `readlink` without traversal. The scanner rejects directory entries with either compressed or uncompressed payload, members above 512 KiB, archives above 4 MiB scanned content/metadata and declared compression above 100:1. History is capped at 128 unique ZIP blobs, 16 MiB each and 64 MiB aggregate; exceeding any cap fails the gate.
...
- Tests: focused public-surface policy 30/30 and complete Python policy 65/65 passed. Fixtures additionally cover malformed-byte-adjacent tokens, secret-bearing member names, central extra fields, a local-header-only extra field with a safe central copy, central/local filename and size disagreement, unclaimed prefix/interior/trailing regions, descriptor disagreement, compressed-stream trailing data, compressed-only directory payloads, newline-containing historical paths, forged-low-count pre-construction entry rejection, current symlink target scanning and merge-result-only archives, alongside the prior alias/tree/comment/control/member/limit/binary boundaries.
...
- Exact merge evidence: clean Codex review comment `5395042872` is bound to exact head `f7ebd012b1`, with review threads 0. Android `32725870456`, Windows `32725870562`, iOS `32725870461` and supply-chain `32725870441` all completed successfully before expected-head squash merge.
- Local boundary: Python policy 39/39, public-surface 395 candidates, exact six product blobs plus #65/#68 preservation, conflict-marker scan and `git diff --check` passed. Gradle 9.7.1 remained unavailable locally; exact hosted Android/Windows runs supply shared/platform compilation and tests.
- Gate: merged source plus exact-head hosted compilation/test evidence. Filesystem/provider imports, recovery, playback, publication and Human evidence remain separate.
...
- Exact merge evidence: clean Codex review comment `5394789436` is bound to exact head `4c8f2c6c42`; workflow runs `32723758695` (Android), `32723758703` (Windows), `32723758687` (iOS), and `32723758693` (supply chain) all completed successfully before the expected-head squash merge.
- Local boundary: Python policy 39/39 PASS; public-surface 394 candidates PASS; six Android XML files parsed; wrapper SHA-256 `7a9ce74cff467ca1bf60a4fcd9f05185acceda4d0f382434d393e17864262c5d` and wrapper text policy matched; all four Desktop product/test files equal the exact reviewed tree; `git diff --check` PASS. Gradle 9.7.1 remained unavailable locally; exact hosted Windows run `32723758703` supplies the compilation/test gate.
- Gate: merged source plus exact-head hosted compilation/test evidence. Physical Windows audio, real scheduling latency, device loss, packaging, provider, publication, and `HUMAN_GO` remain unclaimed.
