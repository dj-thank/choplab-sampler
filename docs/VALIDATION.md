...
- 2026-08-16 TalkBack continuation: the actual TalkBack service plus touch exploration produced a visible accessibility-focus ring on the source waveform and one-finger next traversal. The physical tree exposed E and chop markers but omitted S when two markers clustered at the start. A local candidate now separates adjacent marker 48 dp semantics into top/center/bottom lanes and assigns explicit traversal indices; clean unit/lint/app/androidTest builds pass, but USB loss interrupted the corrected APK rerun. Spoken labels, complete S/E/chop order, and microphone contention therefore remain `DEVICE_UNVERIFIED`.
