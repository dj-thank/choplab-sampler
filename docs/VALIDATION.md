...
- Integration boundary: reachable product commit `99bd386dad7204dd5eec7d8d30cb6de0297b62a0`, tree `dec79d184ab72ee662c261b0b03f4c8cde9f3d34`, joins the repaired PR #64 lineage with merged `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`. The product tree retains #65 Desktop transport readiness, #66 fractional timing, #67 Desktop import sample-rate admission, #68 Android realtime terminal-sample retirement and #70 Android import-name persistence; the following evidence commit is documentation-only.
...
- Local evidence: Python policy 39/39, public-surface scan 394 candidates, conflict-marker scan and `git diff --check` PASS after integrating exact `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`. The two transport-only #65 files match merged main byte-for-byte and its controller/test changes are present beside the recovery regressions. Gradle 9.7.1 cannot create its default-cache lock directory here, so the focused Desktop tests and full Windows workflow require hosted read-back on the published exact head.
