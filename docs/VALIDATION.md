...
- Product source: reachable integration `92a28999992e8f5b74666a26a2e0a2a994b22ca5`, tree `337b9f722d7fde521e1d37cdcd6f11dfc44c6f30`, with parents repaired policy `c4096092a0d000b50db702c9f5556c8227fa7ddd` and squash-merged `main@029500ac63fe521814530acf4d70cab78365c9fd`; the final evidence commit is documentation-only. #74 runtime/test/docs bytes are retained.
- Layout contract: bounded EOCD/central/local parsing precedes `ZipFile` allocation. Every local record, metadata field, compressed span and optional descriptor must claim one contiguous byte range. Signed and signatureless descriptors are both checked against central CRC/sizes, then structural boundaries disambiguate dual matches. Local/central filenames, flags, methods and non-descriptor CRC/sizes must agree before classification.
- Content contract: an independent bounded decoder consumes the exact stored/deflate/BZIP2/LZMA span and requires exact decompressed size and CRC. It rejects trailing input and output beyond a forged declared size instead of trusting `ZipExtFile` truncation. Safe member content and both metadata copies share the 4 MiB budget; member output is capped at 512 KiB and declared compression at 100:1.
- Reach/history contract: current symlink targets are read without following them. NUL-safe per-parent merge history enumerates regular ZIP blobs before deduplication and applies 128-blob / 16 MiB each / 64 MiB aggregate materialization caps. Binary/audio payload exclusion and raw-container non-rescanning remain intact.
- Tests: focused policy 33/33 and complete Python policy 68/68 passed. New fixtures bind forged short output, signed/signatureless descriptor collision and all four supported methods in addition to the prior forged-count, metadata, ownership-gap, trailing-stream, directory-payload, symlink and historical edge cases.
- Static gate: the integrated 396-candidate current/history scans, both known historical ZIP blobs, conflict-marker scan and `git diff --check` passed. This Python-only delta claims no Gradle, device, provider, public-release or Human result; exact-head hosted CI and clean review remain required.

## Reverse PAD resampling tail merged-main read-back — 2026-08-24

- Product source: squash-merged `main@029500ac63fe521814530acf4d70cab78365c9fd`, tree `1ec604a2564ad39ff5c44d50f835d237c1ac6639`, from exact head `556cd0fa2c37950f349c970265a67afb82a3da36`; product `1b8b424036f76dabbb0671f4954ed079f32b9247`, tree `614c93f60c1e55e8df097302ef0e22250ddf7d37`.
...
- Exact merge evidence: clean Codex review comment `5396238626`, zero unresolved threads and successful Android `32735297596`, Windows `32735297567`, iOS `32735297469` and supply-chain `32735297580` workflows were bound to exact head before expected-head squash merge.
- Gate: merged source plus exact-head hosted compilation/test evidence. No listening, physical output device, public artifact or Human result is claimed.
