...
- Product/policy source: reachable policy product `7e98d9c8eddaa6ee28326bcf9e7ab579e6a90c1d`, tree `4d43ec3635797c064f2b4c787cd102f37ccec7d1`, advancing prior exact policy head `e827f48c8c0ffa48b6d04656bdc0785ac08fbc63` on squash-merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; this section is in a later documentation-only commit. Merged #65/#68/#70/#71/#73 runtime, tests and documentation are retained.
- Contract: a bounded end-of-central-directory preflight enforces central-directory byte bounds, parses each central record and compares the actual count before `ZipFile` can allocate `ZipInfo` objects, and rejects multi-disk/ZIP64 sentinel cases plus unclaimed prefix/trailing bytes. Local filenames, flags, compression methods and non-descriptor CRC/sizes must match the central copy before suffix policy or payload bounds are trusted. Safe-named ZIP member text, archive comments, central-directory filenames/comments/extra fields and local-header filenames/extra fields in the current tree and reachable historical ZIP blobs are then scanned with the ordinary secret patterns under one byte budget. Replacement decoding preserves ASCII tokens beside malformed bytes. Historical candidates are derived from NUL-delimited per-parent raw path/blob records before blob-ID deduplication and accept only regular-file modes. The scanner rejects payload-bearing directory entries, members above 512 KiB, archives above 4 MiB scanned content/metadata and declared compression above 100:1. History is capped at 128 unique ZIP blobs, 16 MiB each and 64 MiB aggregate; exceeding any cap fails the gate.
...
- Tests: focused public-surface policy 26/26 and complete Python policy 61/61 passed. Fixtures additionally cover malformed-byte-adjacent tokens, secret-bearing member names, central extra fields, a local-header-only extra field with a safe central copy, central/local filename and size disagreement, unclaimed prefix/trailing regions, newline-containing historical paths, forged-low-count pre-construction entry rejection, ZIP-named symlinks and merge-result-only archives, alongside the prior alias/tree/comment/control/member/limit/binary boundaries.
