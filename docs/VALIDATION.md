...
## Android microphone responsive startup cancellation candidate — 2026-08-24

- Product source: reachable review follow-up `9b092facd2f59c61b7eccde832eee6b4ac1a4345`, tree `d3bdae29429b10408859d6e4933a796c6ca56392`, advancing latest-main integration `7c2ecbcc5f4aba9083a03f38905f6196913beaef` on exact merged `main@029500ac63fe521814530acf4d70cab78365c9fd`; the later evidence commit changes documentation only. ViewModel/recorder/recorder-test/session-test/session-reducer blobs are `e6093e8680afde4b36f4998ef32575e5869c6be0`, `4fbbdab4f5e0b3aee01ed0edaffba5d623ef407f`, `4f29a59e3543997d200b563cb4b5fced5da8a942`, `a91ccc9deb840aee739efd7ae932c5db15fd1cf3` and `3a91cbb5715e77641ac1b5c0fd9925d3b5891245`.
- RED contract 1: a fake `startRecording()` can block indefinitely, while production Android called `MicrophoneRecorder.start()` directly on the Main callback path. A separately threaded test did not prove the user could reach STOP when the caller itself was blocked.
- RED contract 2: pending cancellation synchronously called `stop()` before `release()`. Android framework start/stop/release can serialize on the same recording-state lock, so STOP could block before its timeout and the fake's immediately returning `stop()` did not model production.
- Repair contract: caller-thread admission publishes the generation/file/latch boundary and launches daemon startup before returning. Actual native success uses `onStarted`, and `confirmRecordingSessionStarted` accepts only the matching STARTING session. Pending STOP claims exact identity, calls no pending `stop`, launches an unjoined daemon `release`, checks the startup latch Boolean and fails closed on timeout while `startCallActive` continues rejecting replacements until real unwind. `stopAsync` reuses the same synchronous claim but runs bounded wait, active native stop, worker join and completion on a daemon thread; RESET and `onCleared()` no longer execute those operations on Main/lifecycle.
- Deterministic regressions: factory-blocked startup returns to its caller and cancels before native start; blocked native start plus independently blocked release still lets STOP finish with `stop/release/read = 0/1/0`; a 25 ms injected bound proves timeout while both native calls remain blocked; replacement admission fails in both unwind and timeout windows. The lifecycle regression requires `stopAsync` to return with `isRecording=false` while completion remains pending. Cancellation removes the temporary WAV. The shared state test proves late success cannot change STOPPING or recreate Idle.
- Local PASS: Python policy 39/39; public-surface 396 current-tree candidates; `./scripts/doctor.sh`; conflict-marker scan; `git diff --check`. `validate_project.sh` passed public-surface and executable-mode checks before Gradle provisioning failed because the uncached 9.7.1 distribution host is unreachable. No Kotlin/Gradle or device PASS is claimed locally.
- Gate: source/static latest-main candidate. Fresh exact-head hosted Android/shared tests, all relevant workflows and clean Codex re-review are required. No physical microphone timing, audio quality/content, route-loss, publication or Human evidence is inferred.

## Reverse PAD resampling tail candidate — 2026-08-24

- Product source: reachable commit `1b8b424036f76dabbb0671f4954ed079f32b9247`, tree `614c93f60c1e55e8df097302ef0e22250ddf7d37`, advancing the product lineage rooted at exact merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; the later evidence commit changes documentation only. Renderer/test blobs are `ffd0be29db89d9ec75d553ab88c6d2d832e77e75` / `315ec9b078f9a8ed76f1783dce53877cbb430a53`; Desktop adapter/test blobs are `95a69a20e1e7c1667bcfa0f759d902eea73327fc` / `e4ccb83e52cdbde65f28d4cdf020cae7ffa72f2f`.
- RED arithmetic: for `[100, 164)`, source 48 kHz and output 60 kHz, `sourceStep=0.8`. Reverse starts at 163 and has 79 valid positions through 100.6; position 99.8 is outside the range. The old forward-derived `ceil(64 / 0.8)` allocated 80 frames and left its broken-out final element as zero.
- Repair contract: reverse one-shot/gate counting and rendering both use `VoicePlaybackCursor`, so the host follows realtime's repeated floating-point advance order. Counting occurs before allocation and the cursor is reset for the PCM pass, avoiding an oversized array and trim-copy. Reverse LOOP retains main's direct positions and finite allocation; it does not use the new cursor. `JavaSoundWavPlayer` passes its effective `forceLoop` mode into the pure renderer before creating/repeating the Clip, preventing the non-loop repair from shortening an intended loop. Forward allocation and all PCM interpolation, tone, gain and boundary-envelope calculations are unchanged.
- Regressions: `reverseResamplingStopsAtTheSameFrameAsTheRealtimeCursor` requires 79 realtime/host frames and a nonzero last valid PCM. `reverseOneShotUsesCursorRoundingAtTheStartBoundary` binds the review's two-frame 8→48 kHz / pitch −12 case to 12 frames. `reverseLoopKeepsTheLegacyNonIntegralRenderBoundary` uses the reported two-frame pitch −5 shape and requires 2 one-shot frames versus main's three-element LOOP render with terminal boundary zero rather than cursor-wrapped PCM. `JavaSoundWavPlayerTest.forceLoopKeepsTheLegacyNonIntegralRenderBoundaryWhenThePadIsOneShot` binds the adapter to the same legacy boundary without opening audio hardware. Arbitrary-pitch loop phase continuity and playhead parity are explicitly outside this non-loop repair.
- Local PASS: arithmetic reproduction; Python policy tests 39/39; public-surface current/history over 396 candidates; conflict-marker scan; `git diff --check`. Focused `:jvm-core:test` could not start because Gradle 9.7.1 is not cached and the distribution host is unreachable.
- Gate: source/static candidate. Hosted Windows/JVM test execution, all required workflows and exact-head clean review remain required. No listening, physical output device, public artifact or Human result is claimed.

## Desktop recorder startup cancellation merged — 2026-08-24

- Product source: merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`, tree `6c00c73d23ac419f12162f3f465812136b403252`; product repair `27283f8bc6ace63a27a9ea84e60db2abee5b4bd6` / tree `4ffc5f746f60cc67b71c29bb1f8a5b5db1a227ad` remains the runtime/test anchor.
...
- Provider correction: exact head `9a844c667e7372df970004aa1583ffbc3c6d6ceb`, Android run `32713574905` / job `97389990084`, failed compilation at `FirstScreenFlowDeviceTest.kt:187` because `longClick` lacked its `androidx.compose.ui.test.longClick` import. The local candidate adds the API import proven by the existing `SourceWaveformDeviceTest` usage; hosted read-back is still required.
