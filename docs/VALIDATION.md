...
- Tests: focused public-surface policy 23/23 and complete Python policy 58/58 passed. Fixtures additionally cover malformed-byte-adjacent tokens, secret-bearing member names, central extra fields, a local-header-only extra field with a safe central copy, newline-containing historical paths, pre-construction entry rejection, ZIP-named symlinks and merge-result-only archives, alongside the prior alias/tree/comment/control/member/limit/binary boundaries.
