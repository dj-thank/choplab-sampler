...
- Product source: reachable commit `6151317924ebe0e5dc33e0ba905ca9b65a7d34e0`, tree `73d6f687067b88e89f9abc85f1051f2e17bdbc66`, advancing the product lineage rooted at exact merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; the later evidence commit changes documentation only. Renderer/test blobs are `784a329675d896086581f405bf64adb356cc23fb` / `7c38837f9123821235c7a5752034e9586b2a13a8`.
...
- Repair contract: reverse counting and rendering both use a one-shot `VoicePlaybackCursor`, so the host follows realtime's repeated floating-point advance order. Counting occurs before allocation and the cursor is reset for the PCM pass, avoiding an oversized array and trim-copy. Forward allocation and all PCM interpolation, tone, gain and boundary-envelope calculations are unchanged.
- Regression: `PadPcmRendererTest.reverseResamplingStopsAtTheSameFrameAsTheRealtimeCursor` independently advances `VoicePlaybackCursor`, requires 79 realtime frames, requires the host PCM length to match, and requires its last valid PCM to be nonzero.
