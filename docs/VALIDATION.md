...
- Product source: reachable commit `f2d4952f581da75f729249bbff949120b514a395`, tree `6e40fe66f9eb7c323e12d829968a314371b98fab`, advancing the product lineage rooted at exact merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; the later evidence commit changes documentation only. Renderer/test blobs are `2ab9a60e18f6d0db763ae3b5649b6e56cbcbb89a` / `689c9890d6bff3592cc3cb9dfbec9ef3e2bef96b`; Desktop adapter/test blobs are `95a69a20e1e7c1667bcfa0f759d902eea73327fc` / `1ebe6b14128a5ff35714d4e5108c5e2f5b662a1a`.
...
- Repair contract: reverse one-shot/gate counting and rendering both use `VoicePlaybackCursor`, so the host follows realtime's repeated floating-point advance order. Counting occurs before allocation and the cursor is reset for the PCM pass, avoiding an oversized array and trim-copy. Reverse LOOP uses its LOOP cursor with the full-range cycle count. `JavaSoundWavPlayer` passes its effective `forceLoop` mode into the pure renderer before creating/repeating the Clip. Forward allocation and all PCM interpolation, tone, gain and boundary-envelope calculations are unchanged.
- Regressions: `reverseResamplingStopsAtTheSameFrameAsTheRealtimeCursor` requires 79 realtime/host frames and a nonzero last valid PCM. `reverseOneShotUsesCursorRoundingAtTheStartBoundary` binds the review's two-frame 8→48 kHz / pitch −12 case to 12 frames. `reverseLoopKeepsTheFullRangeCycle` requires the two-frame pitch −12 one-shot/LOOP lengths to remain 3/4. `JavaSoundWavPlayerTest.forceLoopRendersTheLoopCycleEvenWhenThePadIsOneShot` binds the adapter's effective mode to 3/4 without opening audio hardware.
