...
## Current snapshot — 2026-09-03 release hardening integration

GitHub `main@bed7a550a71b1ae91556b2b2af25d7c482083c98` contains the H13 correction, stable Compose 1.11.1, v0.17.2 signer recovery, Android audio-fidelity repair, complete artifact/history scanning, and the reviewed Windows/UI/startup work. This sole-writer branch adds the final CI/release identity, immutable publication and current-product Definition of Done controls before final-main verification and the new `v0.17.2` release. Existing tags and the `v0.17.0` Release remain unchanged; failed `v0.17.1` is never moved or reused.

- Verification workflows expose unique required job names, run every pull request and only `main` pushes, and keep Windows verification mandatory without a path filter.
- Android instrumentation XML must contain tests and zero failure/error/skip; debug APK verification binds the signer to a pre-build certificate observation. iOS derives version/build from `gradle.properties` instead of a duplicate literal.
- Tag release metadata requires an annotated `v${version}` object, exact `GITHUB_SHA`, monotonic version/build history and reachability from `main`; the final manifest permits only the declared Android debug, unsigned iOS Simulator and CycloneDX SBOM assets. Windows remains verification-only.
- [`DEFINITION_OF_DONE.md`](DEFINITION_OF_DONE.md) now defines the current Android/Windows/iOS-preview product. Historical Pro/MIDI/stems/Oboe/effects material is explicitly future non-scope rather than an unfinished second product line.
- Local release-hardening receipt: [`outputs/release-hardening-20260903.md`](../outputs/release-hardening-20260903.md). Full policy 242 and current/history public scans 493 each pass; release metadata resolves to `0.17.2 (29)` / `v0.17.2`.
- Physical Pixel 9a update: [`outputs/android-pixel9a-v0172-20260903.md`](../outputs/android-pixel9a-v0172-20260903.md). Exact `main@bed7a55` tree installed in place as `0.17.2 (29)` with signer continuity, unchanged project-storage aggregate before/install/launch, cold launch 930 ms, top-resumed activity and zero focused fatal matches. Subjective audio remains a separate Human listening gate.
