...
Pixel 9a `5A121JEBF08094` is not connected. This establishes local and focused emulator evidence only; physical retained-data install, actual route loss, another media app/call focus contention, microphone/system-capture behavior, subjective no-double-audio listening, and `HUMAN_GO` remain pending. Public provider and Release evidence for 0.13.1 are recorded only after exact-commit CI and public asset readback.
