...
- Observed at: `2026-08-24T12:24:23Z`.
- Source state: reachable integration product commit `9bb8eff6d0c43b7648869004eeddec82f93ed9f2`, tree `59e8ef0198ade5d0fa19636745ebc3c52e9f3c42`, joins the repaired PR #64 lineage with merged `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`. The product tree preserves #71 shared Android/Desktop import-name admission, #65 Desktop transport readiness, #66 fractional timing, #67 Desktop import sample-rate admission, #68 Android realtime terminal-sample retirement and #70 Android provider/URI name selection; the final follow-up is documentation-only and binds this immutable source.
- Reproduced defects: closing inside the 900 ms debounce window could lose the last edit. In addition, `Future.cancel(false)` may report success after an autosave body has started, so close could enqueue the same snapshot twice and rotate recovery generations unnecessarily; close also waited for persistence before stopping live audio resources. An admitted `loadWav` completion could remain blocked before state publication while close captured the old state, and a transient store failure was still treated as completed, preventing an equal-revision close retry. Later review found that close could capture the initial loading state while startup recovery still owned the persistence executor and overwrite the recovered project. Re-review then found four remaining paths across two rounds: a failed recovery's empty error placeholder could supersede intact archives; close waited for recovered-audio hydration that would immediately be discarded; hydration could claim a newer operation epoch after publication and invalidate a user load/open started in that gap; and an output-device failure during hydration escaped without an actionable state.
- Repair: the controller explicitly tracks autosave work as `SCHEDULED`, `RUNNING`, `COMPLETED` or `CANCELLED`, and each work owns immutable state, ProductionSession revision and save-success outcome. Startup recovery separately records pending/success/failure plus the failure revision. It claims one project-operation epoch before the controller escapes construction, publishes the recovered state only while that epoch is current, and reuses the same epoch for asynchronous PCM hydration; a user load/open begun after publication therefore wins. Hydration is skipped after close and catches output-device failure while retaining the source and publishing the same actionable error as normal WAV import. Close then invalidates project operations and waits for any already admitted publication before capturing state. It never saves an unchanged recovery-error placeholder, stops transport, scratch, recorders and player before the autosave wait, replaces scheduled work or awaits running work, retries an unsuccessful latest save once, and writes a close-owned snapshot when no work exists. A strictly newer close revision still receives one follow-up after an older successful body. Repeated close remains idempotent.
- Regression: the original 60-second-debounce test still requires immediate close to recover BPM 137. A blocked startup load keeps close waiting, requires recovered BPM 127 in both generations rather than stale initial BPM 92, and proves recovered PCM hydration is skipped after close with zero engine loads. A corrupt startup archive is restored after the visible recovery failure; close must leave its exact original bytes as primary and create no backup. An unconfined state observer starts a large user WAV load exactly when recovered state is published and requires only that user audio to reach the engine and project; a separate engine failure requires the recovered source to remain with a visible device error. A blocked running save proves BPM 143 is written exactly once by requiring only the primary generation, and a second blocked-save test proves live resources stop while close is still waiting. The asynchronous project-open race requires newer BPM 151 after an older BPM 143 save. A separate `loadWav` race blocks inside `player.loadPcm` before publication and requires persisted source name/frame/range; its former `projectLaunchTarget` assertion was invalid because the archive intentionally omits runtime routing hints. A deterministic file-versus-directory failure then clears and requires close to retry BPM 149 without rotating a backup.
- Hosted RED: exact head `28c13a2da58e4f3f66e26eb503d507208c32fa7b` passed Android, iOS and supply-chain workflows, but Windows run `32718029950` / job `97403336494` failed the non-persisted `projectLaunchTarget` assertion at `DesktopSamplerControllerTest.kt:539`. The test now checks only persisted source invariants; the corrected successor's hosted result is recorded below.
- Superseded-head GREEN / review RED: exact head `c17ab19dd75145d2477119ab508448bd3fecf318` passed Android run `32723877713`, Windows run `32723877725`, iOS run `32723877824` and supply-chain run `32723877733`, then exact-head Codex review identified the hydration ownership and device-failure paths above. The repaired head requires its own hosted read-back.
- Prior repaired-head GREEN: exact head `ce201141da543540e23b31524ac8e9e0ef0c169d` passed Android run `32726261739`, Windows run `32726261784`, iOS run `32726261818` and supply-chain run `32726261852`. Main then advanced through #71, so those receipts remain bound to the pre-integration head.
- Fresh local evidence after integrating `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`: 39 Python policy tests, public-surface scan over 395 candidates, conflict-marker scan and `git diff --check` pass. All six #71 product/test blobs and its hardening-plan blob are byte-identical to merged main; the two #65 transport-only files remain exact, and its controller/test changes stay reconciled with the recovery repairs. #66/#67/#68 behavior remains inherited unchanged. The Gradle 9.7.1 distribution is not cached and its lock directory is not writable in this Linux environment, so the focused Desktop tests, full JVM gate and packaged Windows runtime remain hosted/local-machine verification boundaries.
- Gate ceiling: latest-main source/static evidence plus revision-bound prior-head hosted evidence only; current provider CI, Windows app-image execution, power-loss behavior and `HUMAN_GO` are not inferred.
...
- Source state: reachable integration product `e0d3fa1df5862bcfa038812bb12ecf6d2c45911e`, tree `1558c4a2331e8ef3a7b2809b38f264e671c188a6`, with parents prior clean PR head `d2c99fc2d4bb9c84bf6366a7f2568cca88294422` and merged `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`; subsequently squash-merged through PR #71 as `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`. Its shared, Android and Desktop product/test files are retained byte-for-byte in the current product above.
- Main preservation: #65 controller/transport/test behavior remains present; its transport-only blobs remain exact `c92613c` / `073696e`. #68 runtime/test blobs remain exact `de686ba` / `9d51556`, and #70 provider/URI name-selection behavior and archive regression remain intact through the shared seam.
...
- Regression: shared common tests bind candidate priority, whitespace-only bounded-prefix fallback and surrogate-safe truncation. Desktop feeds a 240-space prefix plus visible suffix, requires `sample`, and round trips the decoded state through `ProjectArchiveCodec`; the existing Android archive regression exercises the same shared function.
- Prior exact-head evidence: `d2c99fc` received a clean Codex review with zero threads and passed Android `32724477061`, Windows `32724477012`, iOS `32724476941` and supply-chain `32724476968`. Those receipts are revision-bound and do not replace hosted execution for the current #64 head.
- Local integration evidence: Python policy 39/39, public-surface 395 candidates, exact product/main blob comparisons, conflict-marker scan and `git diff --check` passed before merge; the current product repeats those static gates after exact `main@dfcd9d8` integration.
- Gate ceiling: merged source plus revision-bound hosted evidence only. Real filesystem/provider imports, autosave recovery, device playback, publication and `HUMAN_GO` remain unclaimed.

## Previous snapshot — 2026-08-24 desktop transport step-zero ordering candidate

- Observed at: `2026-08-24T20:47+09:00`.
- Source state: reachable integration product commit `08fb123888fb840496d34e4ba7a586013e1305f6`, tree `dc9a9e8563e18168c5d20af9084ffaab01f0f742`, with parents prior exact PR head `5d630c8a769e4b840bba9914f59bc0ec1c705638` and merged `main@333088147cdc77932efc41b90a08eb37e1c1cf42`; subsequently squash-merged as `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`. Its four Desktop product/test blobs and merged #66/#67/#68/#70 histories are retained in the current product above.
...
- Prior exact-head evidence: remote head `0a9def1816bffc903319b5358249f71b43f4c2cf` received a clean exact-head Codex re-review and workflow runs `32720971504` (Android), `32720971498` (Windows), `32720971362` (iOS), and `32720971385` (supply chain) all completed successfully.
- Gate ceiling: the merge and retained source are revision-bound evidence only. Windows scheduling beyond the deterministic contract, audible timing/latency, device removal, provider, public artifact and Human acceptance are not inferred.
...
- Import policy: materialized mono frames are bounded by `min(30,000,000, sampleRate × 600)`. Android reapplies the ceiling when the effective output rate arrives; Desktop applies it to declared and unknown-length streams.
