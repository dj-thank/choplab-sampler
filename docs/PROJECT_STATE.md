...
## Current snapshot — 2026-08-24 Android microphone responsive startup cancellation candidate

This snapshot records one bounded Android microphone lifecycle correction integrated with the exact main that merged the reverse PAD resampling-tail repair. It changes microphone startup/cancellation ownership, the Android ViewModel's native-start confirmation boundary, the shared recording-session confirmation reducer and focused host regressions; PCM format, recording budgets, archive schema, playback capture and the merged reverse renderer are unchanged.

- Observed at: `2026-08-24T23:29+09:00`.
- Source state: reachable review follow-up product `9b092facd2f59c61b7eccde832eee6b4ac1a4345`, tree `d3bdae29429b10408859d6e4933a796c6ca56392`, advancing integration product `7c2ecbcc5f4aba9083a03f38905f6196913beaef` on exact merged `main@029500ac63fe521814530acf4d70cab78365c9fd`, tree `1ec604a2564ad39ff5c44d50f835d237c1ac6639`; the later evidence commit is documentation-only. ViewModel/recorder/recorder-test/session-test/session-reducer blobs are `e6093e8` / `4fbbdab` / `4f29a59` / `a91ccc9` / `3a91cbb`.
- Reproduced defects: although a pending input was published outside the lifecycle lock, production still invoked potentially blocking `AudioRecord.startRecording()` synchronously from MainActivity/ViewModel callbacks, preventing the user-visible STOP callback from running on the same main thread. If STOP did run from another thread, it synchronously entered pending `AudioRecord.stop()` and then `release()`; both can serialize with the blocked framework start lock, so STOP could hang before reaching its two-second bound.
- Repair: `start()` now synchronously reserves one generation and publishes its file/latch state, then returns after launching a daemon startup thread for input creation and native start. Native success is reported separately through `onStarted`; Android advances only the exact current `STARTING` session, so a late callback cannot resurrect Idle/STOPPING or restart a vocal Beat loop. STOP atomically claims the pending input, never calls its framework `stop`, hands `release()` to a separate daemon cancellation thread without joining it, and checks the bounded startup wait result. Timeout fails closed, deletes the incomplete WAV and keeps replacement admission blocked until the original startup actually unwinds. RESET and `onCleared()` use `stopAsync`: cancellation ownership and `running=false` are published synchronously, while every native wait/stop/join and completion callback runs on a daemon stop thread.
- Deterministic regressions: one factory-blocked case proves caller return plus pre-native cancellation; one blocked native-start case holds `release()` independently and requires STOP completion while release is still blocked, pending `stop` count zero, release count one, worker reads zero and replacement rejection; a short injected timeout keeps both native start and release blocked and proves bounded failure plus fail-closed replacement. A lifecycle-call regression proves `stopAsync` returns with stopped state visible while its completion remains blocked behind native startup. A shared reducer regression requires native-start confirmation to advance only STARTING and leave STOPPING/Idle byte-for-byte unchanged.
- Main preservation: the integration tree starts from exact `main@029500a`; all nine #74 reverse renderer/adapter/test/documentation/plan paths are retained from that tree, and none overlaps this product delta.
- Fresh local checks: Python policy 39/39, public-surface 396 current-tree candidates, `./scripts/doctor.sh`, conflict-marker scan and `git diff --check` PASS. `validate_project.sh` passed public-surface and executable-mode phases, then the uncached Gradle 9.7.1 distribution could not be downloaded in the network-restricted sandbox; no local Kotlin/Gradle PASS is claimed.
- Gate ceiling: source/static latest-main candidate only. Exact-head hosted Android/shared compilation/tests, all relevant workflows and a clean re-review remain required. Physical microphone lock timing, audio quality/content, route loss, public release and `HUMAN_GO` remain unclaimed.

## Previous snapshot — 2026-08-24 reverse PAD resampling tail candidate
