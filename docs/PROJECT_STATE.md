...
- PR #84 pre-documentary head `a1fdc80b4733734fbe7ed33cd86789dbc5c7428a` / tree `c925978156383a8ec298b6662d26adcc8f3afa69` waits for the Android framework accessibility tree, reacquires laid-out offscreen pointer targets, emits full test stacks, and narrowly classifies only the observed Compose 1.12 `RectManager.remove` → `ImageComposeScene.close` teardown signature. Product behavior and semantics are unchanged.
- Exact `a1fdc80` hosted result: Android, Windows, iOS, and supply-chain push/PR runs 8/8 succeeded. Local JDK 17 H13 target is 25/25 (12 product UI interactions + 1 teardown-classifier harness test + 12 controller tests); AndroidTest Kotlin compile is 42 tasks; policy is 75/75; current-tree public scan is 479 candidates with credential/signing/audio candidates 0.
...
- Current selected execution plan: [H13 GitHub integration and v0.17.1 release](../plans/active/h13-github-release-20260902.md). The current ceiling is scoped PR/provider evidence for `a4030ea`; public binary, physical device/audio, Spotify account/provider, screen-reader speech, and Human gates remain separate.
