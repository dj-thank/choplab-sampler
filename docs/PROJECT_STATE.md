...
The physical Pixel 9a serial `5A121JEBF08094` is not currently enumerated by ADB; only the emulator is attached. Therefore this section establishes `LOCAL_PASS` plus emulator-scoped install/data/launch evidence, not a physical-Pixel `DEVICE_PASS`, subjective no-double-audio listening, microphone/system capture quality, scratch feel, public-release availability, or `HUMAN_GO`. GitHub CI/release and a data-preserving Pixel install remain separate gates.
