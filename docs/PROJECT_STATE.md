...
- Observed at: `2026-08-24T21:25:00+09:00`, after the immutable product commit and its static read-back were available.
- Source state: PR #52 merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`; PR #62, PR #65, PR #66, PR #67, PR #68, PR #70, PR #71 and the intervening bounded improvements are merged through `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887`. The complete executable PR #58 repair is bound to reachable two-parent product commit `986ea4b66e040946ef9722668435aae9a88be7f9`, tree `6b28819d7e5af0a7a94ada63ea91d0c151f7a674`, whose main-side parent is exact `dfcd9d8`; the final follow-up commit only binds documentation to that immutable source anchor. The integrated main side retains Desktop's shared 8–192 kHz import admission, mixes a pooled realtime PAD voice's terminal returned sample before retirement, publishes Desktop transport readiness before worker start so step zero cannot be discarded, and applies one shared nonblank 240 UTF-16-unit display-name boundary to Android provider/URI names and Desktop file names before PCM publication. A prior report names local build/runtime source `43d8ace6aa43f3eb6e3b9dc01ea74604ee600705` / tree `798212c33d1dcc3eb52ea79fb20e13b87a9b2d9a`, but that source is not in the current PR/main ancestry; its receipts are explicitly externally unverifiable historical context and establish no current gate.
...
- Shared navigation: `入れる → チョップ → ビート → 保存` remains the only workflow model. Font scale 1.2+ uses a simplified header, a two-by-two stage strip and multi-line status. CAPTURE scrolls for large text or compact landscape; large-text CHOP and BEAT bodies use explicit 48 dp-safe PAD heights and bounded scrolling so every control remains reachable. CHOP and ONE SHOT actions in those scroll bodies commit on completed tap, including an empty CHOP pad held stationary beyond the long-press timeout. A performance GATE waits through a 120 ms scroll-classification window, then consumes later movement and stays active until the physical pointer-up. Every quick release receives an 80 ms minimum measured from the actual trigger, including releases just after delayed activation, and an older preview timer cannot release a newer retrigger. Movement after activation retains GATE ownership but cancels stationary trim recognition. Parent cancellation before activation sends no PAD action, while pointer-node cancellation after activation releases trigger ownership exactly once. One-finger vertical movement over the CHOP waveform passes through only when the large-text parent scroller exists; without that ancestor it is consumed as a no-op drag so the sibling tap recognizer cannot seek or move a trim boundary on up. Horizontal intent and multi-touch retain waveform pan/zoom. Non-scroll PADs retain press-down response. Compact landscape keeps transient status feedback in the header when the separate status strip is absent.
...
- Review repair through `986ea4b`: all code-applicable GitHub findings are addressed without resolving their threads. Deferred GATE ownership releases on pointer-node cancellation; play-mode changes restart pointer arbitration; post-activation movement is consumed through physical up while suppressing trim after touch slop; stale preview generations cannot cut retriggers; every quick GATE release retains at least 80 ms from its actual trigger; and a held empty CHOP pad completes capture at pointer-up. The waveform detector passes one-finger vertical intent only to the large-text CHOP scroller, consumes it without seeking in non-scroll CHOP/trim contexts, and preserves horizontal and two-pointer transforms. Real-pointer regressions retain parent movement plus zero controller calls for CHOP, ONE SHOT and pre-activation GATE swipes, then cover post-activation movement/no-scroll/no-release-until-up, overlapping short-preview generation ownership, quick post-activation release timing, moved-GATE trim suppression, waveform-origin vertical scrolling and non-scroll drag-without-tap. Computed PAD-grid heights retain 48 dp targets; normal-text fixed/responsive paths remain unchanged. Current-container checks pass `git diff --check`, 39 Python tests, parsing of all six Android XML files, the 395-candidate public-surface scan, exact #65/#68/#71 runtime-test blob comparisons and unchanged gesture product/test blobs from the prior reviewed integration. Hosted Android run `32716871329` on prior head `18b815f` compiled product/tests and reached instrumentation, where the large-text target check correctly failed because it measured the inner `B01` text glyph rather than the PAD button. The integrated fixture measures the assigned PAD semantics node and separately asserts the full ONE SHOT/DRM description; fresh exact-head hosted CI is still required.
