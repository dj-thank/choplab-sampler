...
- Observed at: `2026-08-24T22:36+09:00`.
- Source state: reachable policy product `53a2e3f6606d26607398ff1cf34d5708591f1d5d`, tree `3c90b3392bdc9eba07f4697040e901d2ed321f71`, advancing prior exact policy head `4c93bf2dfe6f74e1699694ae9cc5bd8a82cb3cd1` on squash-merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; the later evidence commit is documentation-only. Merged #65/#68/#70/#71/#73 runtime, tests and documentation are retained.
- Archive policy: before `ZipFile` construction, a bounded tail read validates the end-of-central-directory record, rejects multi-disk/ZIP64 sentinels, bounds declared central-directory bytes, and parses every fixed-size central record so a forged low end-record count cannot bypass the 4,096-entry cap. Every clean entry's local-header, metadata, declared compressed-payload and optional data-descriptor ranges must form one exact contiguous ownership chain from byte zero to the central directory; descriptor CRC and sizes must agree with the central record. Each local filename, flag set, compression method and non-descriptor CRC/size tuple must also agree before directory/binary classification or payload bounds are trusted. Member decompression must consume the complete declared compressed span without trailing or unconsumed bytes. Non-binary ZIP members plus archive comments, central-directory filenames/comments/extra fields and local-header filenames/extra fields in both the current tree and reachable historical ZIP blobs are then scanned without extraction. Both metadata copies share the bounded archive budget, so a token present only in the local header cannot hide behind a safe central-directory value. ASCII credentials remain matchable beside malformed bytes through replacement decoding. Historical ZIP versions come from NUL-delimited per-parent raw path/blob change records before blob-ID deduplication, so newline-containing paths, merge-result-only archives and shared blobs with a non-ZIP path hint remain visible, while tree, symlink and gitlink objects never consume the blob cap. Current-tree symlinks are inspected with `readlink` and secret-scanned without following their targets. The scanner fails closed above 512 KiB per member, 4 MiB per archive or a 100:1 declared compression ratio; historical materialization is separately capped at 128 blobs, 16 MiB each and 64 MiB aggregate. Directory-marked entries with either declared compressed or uncompressed payload are rejected before content handling.
...
- Regression scope: UTF-8 BOM, control-containing and malformed-byte-adjacent secret text, secret-bearing member names, archive/member comments, central-directory extra fields, a local-header-only extra field paired with a safe central copy, central/local filename and size disagreement, token-bearing prefix/interior/trailing unclaimed regions, descriptor disagreement, compressed-stream trailing data, newline-containing historical ZIP paths, reachable removed ZIP content whose `rev-list` hint names only a non-ZIP alias, merge-result-only ZIP discovery, ZIP-named tree exclusion, current broken/safe and secret-shaped symlink targets, forged-low-count pre-construction entry rejection, member/aggregate/compression limits, compressed-only directory payloads and UTF-8-decodable raw-container exclusion are direct fixtures. Binary and audio member bodies containing token-shaped ASCII do not produce a content match.
- Fresh local checks: focused public-surface tests 30/30, complete Python policy suite 65/65, current-tree and reachable-history public scans over 395 candidates, conflict-marker scan and `git diff --check` PASS. Both known reachable historical ZIP blobs were included and passed. This Python-only delta requires fresh hosted CI; no Gradle, device, provider, public-release or Human result is claimed.
...
- Source state: squash-merged `main@dfcd9d8871f34ca5ed125c9a1113c6a4dd612887` from exact PR #71 head `f7ebd012b15012a4621c1377db81d58f730257a1`; its pre-merge integration product is `e0d3fa1df5862bcfa038812bb12ecf6d2c45911e`, tree `1558c4a2331e8ef3a7b2809b38f264e671c188a6`.
...
- Exact merge evidence: exact head `f7ebd012b1` received clean Codex review comment `5395042872` with zero threads; Android `32725870456`, Windows `32725870562`, iOS `32725870461` and supply-chain `32725870441` all completed successfully before expected-head squash merge.
- Local boundary: Python policy 39/39, public-surface 395 candidates, exact product/main blob comparisons, conflict-marker scan and `git diff --check` PASS. Gradle 9.7.1 remained unavailable locally; the exact hosted Android/Windows runs above supply shared/platform compilation and tests.
- Gate ceiling: merged source plus exact-head hosted compilation/test evidence. Real filesystem/provider imports, autosave recovery, device playback, publication and `HUMAN_GO` remain unclaimed.
...
- Source state: squash-merged `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874` from exact PR #65 head `4c8f2c6c42f059b685f9c46f37f6bd7e3be4e9fe`; its pre-merge integration product is `08fb123888fb840496d34e4ba7a586013e1305f6`, tree `dc9a9e8563e18168c5d20af9084ffaab01f0f742`. The four reviewed Desktop product/test blobs and merged #66/#67/#68/#70 source, tests and documentation are retained.
...
- Exact merge evidence: exact head `4c8f2c6c42` received clean Codex review comment `5394789436`; workflow runs `32723758695` (Android), `32723758703` (Windows), `32723758687` (iOS), and `32723758693` (supply chain) all completed successfully before expected-head squash merge.
- Local boundary: Python policy tests 39/39, public-surface scan over 394 candidates, six Android XML parses, wrapper checksum/UTF-8 policy, exact equality of all four reviewed Desktop product/test files, and `git diff --check` passed. Gradle 9.7.1 was not cached locally; the exact hosted Windows run recorded above supplies the compilation/test gate.
- Gate ceiling: merged source plus exact-head hosted compilation/test evidence. Windows scheduling beyond the deterministic contract, audible timing/latency, device removal, provider, public artifact, and Human acceptance are not inferred.
