...
- Nested/alias reach: ZIP-compatible nested archives recurse to depth 3 under 64-archive, 16 MiB/member and 64 MiB aggregate compressed/uncompressed bounds; unsupported nested formats fail closed. BOM-marked UTF-16/32 text and safe-named RIFF/AIFF/FLAC/Ogg/CAF/ID3/M4A payloads are detected. Non-commit tree refs are enumerated NUL-safely through `for-each-ref` plus `ls-tree -rz`.
...
- Executable proof: complete Python policy 106 tests passes with one explicit Windows-only symlink-fixture skip on a host lacking privilege. Current/history public scans pass over 465 candidates. Configured validation passes 18 tasks; JVM 88 / 9 suites and Desktop 165 / 24 suites have zero failure/error/skip.
