...
## Current snapshot — 2026-09-02 PR #84 merged / Android audio fidelity

The H13 Desktop long-press correction and stable Compose test runtime are now on GitHub `main`. The current sole-writer candidate fixes the user-reported Android distortion before the remaining security/release/UI branches converge. Historical evidence and the existing `v0.17.0` Release remain unchanged; no `v0.17.1` tag or Release may be created before final-main verification.

- PR #83 exact head `13e41af589ee32018ae6857623b857b9c0356f21` was merged by `dj-thank` at `2026-09-02T08:05:39Z` as `2864117fe3c81b308033155dae337a6030165344` after its exact-head hosted checks succeeded. Its four review-repair receipts remain the detailed correction chain; their former pending statements are historical, not the current frontier.
- Current GitHub `main`: PR #84 head `51238b1f29dc0c9bfb904d569fdf2081b23e56e3` was normally merged as `f6cbfdcc65584264ca7fd1cf7c450e9cab284b14` at `2026-09-02T10:50:42Z`. Merged-main Android `33621558451`, Windows `33621558443`, iOS `33621558483`, and supply-chain `33621558454` workflows all succeeded.
- PR #84 retains Android framework-tree readiness, offscreen pointer-target reacquisition, full test stacks, and bounded asynchronous observation headroom. Its Compose 1.12 exception-classifier experiment is removed; plugin/runtime/foundation/UI are pinned to the previously verified 1.11.1 line, H13 is 24/24, and normal render/close failures remain fatal. The detailed `a1fdc80` → `f47519f` diagnostic chain is preserved in the linked stabilization receipt rather than treated as a current gate.
- Android audio product checkpoint `b63ed650e47c5555f4a328171c222ca4888a88ae` / tree `7ce7caaea092c9331afe2ea2d51b8efdb19895e1` replaces the always-on `x/(1+abs(x))` curve: default gain 0.9 is bit-for-bit linear and only overload enters a C1-continuous knee below 0.98. `SamplerEngine.renderLoop` and `PatternRenderer` retain one shared limiter.
- Import/master hardening checkpoint `9dc71a4652c943ded89c62bb50d9512270182d20` / tree `4d8eb5f49d7608ee1ad2e1a3315d982065587fb2` binds the actual AudioTrack output seam, refuses unsupported or mid-stream PCM encoding changes, rejects non-finite decoded PCM, and removes DC only by an offset that cannot create a clipped sample. It descends from the PR #84 merge through local merge `cfad667b3e51a20d07fe99dc379fa1432d1b6b5b`.
- Review hardening `c5c5bcc692b947e684713f7a4e9b8c3762728f34` keeps the fast 240,000-frame DC probe but confirms the offset against the whole signal before mutation; `7e7c7ae5a5b30f7f3e526ba887825fe60b400fd1` / tree `b45b0d7f8afca04684e2149505cf4b3e869ef2ed` separates encoding validation from stability and adds dense limiter monotonicity/symmetry checks.
- Exact JDK 17 local result for `7e7c7ae`: shared Desktop 87, shared Android host 87, Android unit 288, JVM core 88; failures/errors/skips 0. Android lint, debug APK assembly, and AndroidTest Kotlin compilation also pass; all 99 selected tasks were executed. Earlier uncommitted predecessor runs exhausted host virtual memory in the default 4 GiB Gradle/test-worker configuration; final `7e7c7ae` passed with one worker, 1.5 GiB Gradle heap, 256 MiB serial test forks, and in-process Kotlin compilation. Those crashes are host-resource evidence, not product-test results.
- Version remains `0.17.1 (28)`. No `v0.17.1` tag or Release exists yet. PR #69 security integration, release hardening, UI brush-up, final exact-main gates, and publication read-back remain before the tag; the workflow must rebuild distributed bytes from the tagged merged commit.
- GitHub guard: Desktop verification and Release Windows verification run `:desktop:desktopLongPressUiTest` before packaging. The public Release contract distributes Android debug preview + unsigned iOS Simulator archive only; signed Android and Windows remain verification-only.
- Retained provenance: [H16 package](../outputs/H16-windows-package-20260901.md), [H19 startup](../outputs/H19-windows-startup-smoke-20260901.md), and the [PR #83 / #84 stabilization receipt](../outputs/PR83-postmerge-accessibility-stability-20260902.md). Older hashes do not claim equality with forthcoming `v0.17.1` assets.
- Current selected execution plan: [H13 GitHub integration and v0.17.1 release](../plans/active/h13-github-release-20260902.md), extended below as the single product-convergence plan. The audio candidate ceiling is `LOCAL_PASS` at `7e7c7ae`; PR #84 has separate merged-main provider evidence. Physical audio, Spotify account/provider, screen-reader speech, public binary, and Human gates remain separate.
