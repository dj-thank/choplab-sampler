...
- Pixel `5A121JEBF08094` is now a scoped `DEVICE_PASS`: accepted receipt `work/device-evidence/20260823-025301-8306ed21/manifest.json` binds clean source `8306ed2`, app/test APK hashes and signer, `adb install -r`, six instrumentation tests, autosave preservation, and foreground/rotation/volume restoration. The initial parser-only false stop is retained separately and not promoted. Browser OAuth, real Premium/allowlist/Connect-device behavior, physical audio quality, screen-reader speech, publication, and `HUMAN_GO` are not claimed.
