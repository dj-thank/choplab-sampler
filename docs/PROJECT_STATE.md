...
- Review repair through `8ba549c`: all code-applicable GitHub findings are addressed without resolving their threads. Every paired GATE path retains a token: large-text deferred touch, normal-layout press-down touch and Desktop keyboard. Android allocates the token only inside a reserved bounded-mailbox slot, stores it on the voice and gives sequencer voices tokens from the same lock-free per-PAD sequence; conditional release therefore fades only its own GATE voice. Desktop publishes ownership only after Java Sound successfully creates/starts the voice. All prior pointer, minimum-preview, initiating-pointer and waveform rotation repairs remain. Exact #74 blobs are preserved: `PadPcmRenderer.kt` / test `ffd0be2` / `315ec9b` and `JavaSoundWavPlayer.kt` / test `95a69a2` / `e4ccb83`. Reverse ONE_SHOT/GATE counting follows realtime cursor order, while LOOP/forceLoop retains the merged direct-position finite boundary. Current-container checks pass `git diff --check`, 39 Python tests, six Android XML parses and the 398-candidate public-surface scan. Prior head `b79f49d` passed Android `32734114319`, Windows `32734114246`, iOS `32734114273` and supply-chain `32734114286`; those receipts predate both the ownership and #74 integration, so fresh exact-head hosted execution remains required.
