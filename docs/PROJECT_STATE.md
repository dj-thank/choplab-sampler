...
- Source state: branch `codex/desktop-autosave-close`, reachable product commit `c189667a0040a0a5e875d7aa8392f9f4086b36c0`, tree `2fa94b464268b91f8ee5c5906be5338b3988e4ec`, integrated with merged `main@ae77cd92d3ee14baecc01f4862c639328bae43bb`. The base includes the #61 sample-rate decode limit, #63 release checksum-sidecar hardening and #57 count-resilient AVD summary parser; other worktrees remain untouched.
- Reproduced defects: closing inside the 900 ms debounce window could lose the last edit. In addition, `Future.cancel(false)` may report success after an autosave body has started, so close could enqueue the same snapshot twice and rotate the recovery generations unnecessarily; close also waited for persistence before stopping live audio resources. A later asynchronous project state could be published while that older save was running, then lose scheduling admission after close claimed the lifecycle, leaving only the older snapshot on disk.
- Repair: the controller explicitly tracks autosave work as `SCHEDULED`, `RUNNING`, `COMPLETED` or `CANCELLED`, and each work owns its immutable state plus ProductionSession revision. Close captures the latest owned revision first, stops transport, scratch, recorders and player, then replaces scheduled work or waits for the existing running body. If the close revision is newer, it performs exactly one follow-up save after that wait. Repeated close remains idempotent.
- Regression: the original 60-second-debounce test still requires immediate close to recover BPM 137. A blocked running save proves BPM 143 is written exactly once by requiring only the primary generation, and a second blocked-save test proves live resources stop while close is still waiting. A deterministic asynchronous project-open race now publishes BPM 151 before its schedule is rejected by close, then requires BPM 151 as primary, the older BPM 143 as the single previous generation and no second backup.
