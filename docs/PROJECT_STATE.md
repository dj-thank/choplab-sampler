...
- Review repair through `0af7da6`: all code-applicable GitHub findings are addressed without resolving their threads. Every paired GATE path retains a token: large-text deferred touch, normal-layout press-down touch and Desktop keyboard. Android allocates the token only inside a reserved bounded-mailbox slot, stores it on the voice and gives sequencer voices tokens from the same lock-free per-PAD sequence; conditional release therefore fades only its own GATE voice. Desktop creates/starts Java Sound first, assigns the returned token to that exact active voice, and conditionally closes that voice even after a sequencer or other input starts a newer voice; failed start publishes no token. The focused fake-port regressions bind stale-release exact targeting and failed-trigger preservation. All prior pointer, minimum-preview, initiating-pointer and waveform rotation repairs remain. Exact #74 `PadPcmRenderer.kt` / test blobs `ffd0be2` / `315ec9b` and Desktop adapter test `e4ccb83` are preserved; `JavaSoundWavPlayer.kt` adds only per-voice ownership around the retained `renderDesktopPadPcm` force-loop seam. Reverse ONE_SHOT/GATE counting follows realtime cursor order, while LOOP/forceLoop retains the merged direct-position finite boundary. Current-container checks pass `git diff --check`, 39 Python tests, six Android XML parses and the 398-candidate public-surface scan. Prior head `b79f49d` passed Android `32734114319`, Windows `32734114246`, iOS `32734114273` and supply-chain `32734114286`; those receipts predate the current ownership/latest-main product, so fresh exact-head hosted execution remains required.
