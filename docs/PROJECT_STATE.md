...
## Current snapshot — 2026-09-02 PR #83 merged / PR #84 stabilization

The H13 Desktop long-press correction and `0.17.1 (28)` release preparation are now on GitHub `main`; the current work is the bounded PR #84 test-harness stabilization and documentary closeout before the new immutable Release. Historical evidence and the existing `v0.17.0` Release remain unchanged.
...
- Current GitHub base: `d3291a522699633d5b7426e4fff66ca4f7b55c09` / tree `c664812a0f213bd33532db6f900ebd62685044e4`. It contains PR #83, the setup-java PR #82 merge, and the Compose Multiplatform 1.12.0 PR #80 merge.
- PR #84 first stabilization head `a1fdc80b4733734fbe7ed33cd86789dbc5c7428a` / tree `c925978156383a8ec298b6662d26adcc8f3afa69` waits for the Android framework accessibility tree, reacquires laid-out offscreen pointer targets, and emits full test stacks. Its Compose 1.12 scene-close classifier was a bounded diagnostic experiment and is superseded by the stable dependency repair below.
- Exact `a1fdc80` historical result: Android, Windows, iOS, and supply-chain push/PR runs 8/8 succeeded. Its temporary local H13 count was 25 including the classifier test; that count is not the current target.
- Post-`a1fdc80` test-only milestones are `75025c51754f8c693b81ed0cef7dd6a852aef7c8` / tree `52b24bfd4a2c9a76fa8a3407b643d930adb730e5` (one stale-recovery close join gains observation headroom) and `e911b417ef1740f69b230731d25c07a158d2147a` / tree `8a28eccc521636f11065cf5f63feca5560687095` (the shared asynchronous `awaitCondition` helper gains the same headroom). Production debounce, recorder, playback, and close deadlines are unchanged.
- Integrated PR #84 head `a4030ea8973b278da1e9e04a37231b8178d32d4b` / tree `e2c36cd58008af6759fd2100bb04bdf529de5696` passed Android, Windows, iOS, and supply-chain push/PR runs 8/8. Fresh review found four SSOT/receipt consistency findings; this documentation successor repairs them, and its own exact-head checks, thread resolution, and fresh review remain the merge gate.
- The next documentation head `f47519f75a9e52e6358643c70dbd8487d2458eaf` reproduced the same Compose 1.12 `RectManager` corruption during `ImageComposeScene.render()` after the close-only classifier had already fired. That head is not release-eligible; a close-only exception filter cannot repair shared scene state.
- Current stable dependency repair: `deed143171806f282768a82d0b9e4fb1fea2a4f8` / tree `bb01345e8bc2c29e5e530229565fb33ea94660ff` pins Compose Multiplatform runtime/foundation/UI and plugin to the previously verified 1.11.1 line, removes the 1.12-only runtime exclusions, and removes the exception filter/classifier. Pointer-target readiness and Android framework-tree readiness remain. JDK 17 local results: H13 24/24, Desktop 180/180, `installDist` PASS, AndroidTest Kotlin compile 50 tasks, policy 75/75, and current-tree public scan 479 with credential/signing/audio candidates 0.
- Version remains `0.17.1 (28)`. No `v0.17.1` tag or Release exists yet. The tag may be created only after PR #84 is merged and that exact merge is reachable from `main`; the workflow must rebuild distributed bytes from the tagged merged commit.
...
- Current selected execution plan: [H13 GitHub integration and v0.17.1 release](../plans/active/h13-github-release-20260902.md). The current code ceiling is `LOCAL_PASS` at `deed143`; scoped PR/provider evidence remains historical for `a4030ea` and must not be transferred to the current bytes. Public binary, physical device/audio, Spotify account/provider, screen-reader speech, and Human gates remain separate.
