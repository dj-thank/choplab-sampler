...
- Observed at: `2026-08-24T22:57:48+09:00`. GitHub recorded immutable product commit `74f2cee837573fb193f1a23b004b8bebf798309a` at `2026-08-24T13:57:36Z`; this observation and static read-back occurred 12 seconds later.
- Source state: PR #52 merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`; PR #62, PR #65, PR #66, PR #67, PR #68, PR #70, PR #71, PR #73 and the intervening bounded improvements are merged through exact `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`. The complete executable PR #58 repair is bound to reachable product commit `74f2cee837573fb193f1a23b004b8bebf798309a`, tree `385cafba8889c4c3f88dfb7abe0077882f75ea0b`, with parents prior PR head `b79f49d34120bea7d3cce34dc1b60c7753ef42ae` and exact main `a0b356c`. The final follow-up commit only binds documentation to this immutable source anchor. The integrated main side retains Desktop's shared 8–192 kHz import admission, mixes a pooled realtime PAD voice's terminal returned sample before retirement, publishes Desktop transport readiness before worker start so step zero cannot be discarded, applies one shared nonblank 240 UTF-16-unit display-name boundary to Android provider/URI names and Desktop file names before PCM publication, and lets STOP cancel a blocked Desktop recorder startup before any worker or read can escape. A prior report names local build/runtime source `43d8ace6aa43f3eb6e3b9dc01ea74604ee600705` / tree `798212c33d1dcc3eb52ea79fb20e13b87a9b2d9a`, but that source is not in the current PR/main ancestry; its receipts are explicitly externally unverifiable historical context and establish no current gate.
...
- Review repair through `74f2cee`: all code-applicable GitHub findings are addressed without resolving their threads. Deferred GATE ownership releases on pointer-node cancellation; play-mode changes restart pointer arbitration; post-activation movement is consumed through initiating-pointer up while suppressing trim after touch slop; every quick GATE release retains at least 80 ms from its actual trigger; and a held empty CHOP pad completes capture at pointer-up. Every paired GATE path now retains a token: large-text deferred touch, normal-layout press-down touch and Desktop keyboard. Android allocates the token only inside a successfully reserved bounded-mailbox slot, stores it on the resulting voice, and gives sequencer voices tokens from the same lock-free per-PAD sequence; a conditional release therefore fades only its own GATE voice and cannot truncate a newer pattern voice. Desktop publishes ownership only after Java Sound successfully creates/starts the voice, so a failed retrigger cannot strand the older owner. Preactivation movement beyond touch slop or outside the PAD cancels without audio, and only the initiating pointer controls quick release, activated release and long-press eligibility. The waveform detector passes one-finger vertical intent only to the large-text CHOP scroller, consumes it without seeking in non-scroll CHOP/trim contexts, and claims horizontal, pinch and rotation movement so a rotation-only gesture cannot fall through as a tap. A new real-pointer normal-text regression, Android ownership/admission/sequence unit regressions, Desktop key-token coverage and a Desktop failed-trigger regression join the earlier parent-scroll, preview timing, cancellation, pointer and no-trim cases. Computed PAD-grid heights retain 48 dp targets. Current-container checks pass `git diff --check`, 39 Python tests, parsing of all six Android XML files and the 398-candidate public-surface scan. All six #71 shared/Android/Desktop product-test blobs and both #73 recorder runtime-test blobs match exact `main@a0b356c`; #68's terminal-sample logic and exact parity test remain, while its runtime file adds only the reviewed voice-ownership path. #65's `DesktopTransport.kt` and `DesktopTransportTest.kt` remain exact while its controller/test retain readiness/step-zero coverage and add ownership failure coverage. Prior head `b79f49d` passed Android `32734114319`, Windows `32734114246`, iOS `32734114273` and supply-chain `32734114286`; those receipts predate `74f2cee`, so fresh exact-head hosted execution remains required.
...
- Source state: reachable integration product commit `08fb123888fb840496d34e4ba7a586013e1305f6`, tree `dc9a9e8563e18168c5d20af9084ffaab01f0f742`, with parents prior exact PR head `5d630c8a769e4b840bba9914f59bc0ec1c705638` and merged `main@333088147cdc77932efc41b90a08eb37e1c1cf42`. PR #65 was subsequently squash-merged as `main@3072eedd84b357f4ccd22c611dcc7b7f22f92874`. Current PR #58 product `74f2cee837573fb193f1a23b004b8bebf798309a` retains its `DesktopTransport.kt` and `DesktopTransportTest.kt` blobs exactly together with merged #66 timing, #67 Desktop import boundary, #68 realtime PAD retirement, #71's shared Android/Desktop import-name source/tests/docs and #73's recorder cancellation source/test. Its controller and controller test preserve readiness/step-zero behavior while adding cross-input trigger ownership; each later evidence commit is documentation-only.
