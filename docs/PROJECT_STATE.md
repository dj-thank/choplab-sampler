...
- Immutable integration product: `e381a7fdeb366fc648b7eb661d5cfa57b5585b48`, tree `6f83d930de9b66a121ed11438de92ed8e8c06dfe`. Exact-main, prior PR #58, PR #64/#76/#77, Wave 10 and Wave 11 heads are all ancestors; `git fsck --connectivity-only --no-dangling` and `git diff --check` pass. A later receipt-only commit may advance the branch without changing these product bytes.
...
- Artifact boundary: Android release metadata is `0.17.0` / code `27`. The 24,274,036-byte release APK (`89cf3234b34bc118dc65a223b3c057726eee83bc04e628433b31b5b44150a458`) passes manifest, permission/export, non-debuggable and alignment policy as an intentionally unsigned CI candidate; `--require-signed` rejects it with exit 1. Windows ProductVersion `0.17.0`, the packaged EXE and the 405-file deterministic app-image manifest read back successfully. No key, signer identity, certificate digest or secret was read or changed.
- Review and remote read-back: local Standards and Spec passes have unresolved findings 0 / 0. At `2026-08-27T02:19:52+09:00`, remote `main` remained `4f56a693`, PR #58 remained clean at `2f6bee3`, repository rulesets were empty and the main protection endpoint reported unprotected. All four hosted workflows succeeded on that predecessor; fresh hosted workflows on the new exact head remain the merge gate.
