...
- Observed at: `2026-08-24T19:44+09:00`.
- Source state: PR #52 merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`; PR #62, PR #66 and the intervening bounded improvements are merged through `main@3260f5cb560e2cbd2d245c7eee6f96ecb3540ddc`. The complete executable PR #58 repair is bound to reachable two-parent product commit `c21b6bebb51448fe3cbc93121abf6cd76bd12c0f`, tree `55aaa4d34d2f4f79aa438494b05386d5d2b6f32d`, whose main-side parent is `3260f5c`; the final follow-up commit only binds documentation to that immutable source anchor. The prior built/runtime anchor remains `43d8ace6aa43f3eb6e3b9dc01ea74604ee600705`; its artifact receipts are not reused as proof for the repair.
...
- Shared navigation: `入れる → チョップ → ビート → 保存` remains the only workflow model. Font scale 1.2+ uses a simplified header, a two-by-two stage strip and multi-line status. CAPTURE scrolls for large text or compact landscape; large-text CHOP and BEAT bodies use explicit 48 dp-safe PAD heights and bounded scrolling so every control remains reachable. CHOP and ONE SHOT actions in those scroll bodies commit on completed tap, including an empty CHOP pad held stationary beyond the long-press timeout. A performance GATE waits through a 120 ms scroll-classification window, then stays active until the real pointer-up; a shorter completed tap receives an 80 ms minimum preview. Parent cancellation before activation sends no PAD action, while cancellation after activation releases trigger ownership exactly once. Non-scroll PADs retain press-down response. Compact landscape keeps transient status feedback in the header when the separate status strip is absent.
...
- Review repair through `c21b6be`: the original GitHub findings plus all five exact-head gesture/evidence follow-ups are addressed. Deferred GATE ownership now releases on pointer-node cancellation; play-mode changes restart pointer arbitration; short GATE taps retain at least 80 ms of preview; all active evidence points to reachable commits; and a held empty CHOP pad completes capture at pointer-up. Real-pointer regressions retain parent movement plus zero controller calls for CHOP, ONE SHOT and pre-activation GATE swipes, ordered ONE SHOT tap actions and unchanged non-scroll press-down playback, while adding separate-frame held GATE release, minimum short-preview duration, cancellation exact-once release and stationary empty-CHOP capture. Computed PAD-grid heights retain 48 dp targets; normal-text fixed/responsive paths remain unchanged. Current-container checks pass `git diff --check`, 39 Python tests, parsing of all six Android XML files and the 394-candidate public-surface scan. Hosted Android run `32716871329` on prior head `18b815f` compiled product/tests and reached instrumentation, where the large-text target check correctly failed because it measured the inner `B01` text glyph rather than the PAD button. The integrated fixture now measures the assigned PAD semantics node and separately asserts the full ONE SHOT/DRM description; fresh exact-head hosted CI is still required.
