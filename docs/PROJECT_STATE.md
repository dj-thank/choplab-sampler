...
Observed/read back at `2026-08-27T10:00:40+09:00`. Existing PR #69 head `1abbd8ff004edceafd81fe5ffa6059e2d3692d85` was merged locally with exact `main@0f5b672afb0e6b67e95290c31900ff5c8abc0ef4` as `fa4e6d2646e44039ce41662ba4c6ae6970ae9dd6`, tree `fafdc23a12c9e5f1e414d8aa2d57da676c382aff`. Product checkpoint is `3ed67101f362e61760c5ceb1839e71e91311aeb5`, tree `70e8f94f4e64cff4e13ea0e474e164632e500770`.
...
- Resource repair: ZIP_LZMA dictionaries are capped at 16 MiB before `LZMADecompressor`; decoded text candidates share a 4 MiB archive-wide declared compressed-input budget in addition to the existing 512 KiB member output, 4 MiB metadata/output, 100:1 ratio and 4,096-entry bounds. `MemoryError` is fail-closed.
- Legitimate binary contract: `.lib` and exact JDK `runtime/lib/ct.sym` entries remain excluded as binary. A main `.app` executable up to the 4 MiB archive budget is fully decoded and secret-scanned. Larger arbitrary safe-named entries fail; only exact `runtime/lib/modules` receives a stored/deflate prefix probe bounded to 64 KiB/member and 256 KiB/archive, and it must expose a structurally consistent JIMAGE header. Secret-shaped labels are redacted before findings reach logs.
- Nested/alias reach: ZIP-compatible nested archives are detected by structure as well as suffix and recurse to depth 3 under 64-archive, 16 MiB/member and shared 64 MiB compressed/expanded-work bounds; unsupported nested formats fail closed. BOM-marked UTF-16/32 text and safe-named RIFF/AIFF/FLAC/Ogg/CAF/ID3/M4A payloads are detected. Non-commit tree refs are enumerated NUL-safely through `for-each-ref` plus `ls-tree -rz`.
...
- Executable proof: complete Python policy 108 tests passes with one explicit Windows-only symlink-fixture skip on a host lacking privilege. Current/history public scans pass over 465 candidates. Configured validation passes 18 tasks; JVM 88 / 9 suites and Desktop 165 / 24 suites have zero failure/error/skip.
...
- Review/gate: local parent Standards/Spec unresolved `0/0`; original dictionary and aggregate-input cases no longer reproduce, supported stored/deflate/BZIP2/default-LZMA controls remain. Strongest fresh result is `LOCAL_PASS`. Existing PR #69 is still remote at its old conflicting head with 27 unresolved threads; no push/reply/merge/tag/Release/device/provider/Human action is claimed by this snapshot.
