...
- Observed at: `2026-08-24T20:51:00+09:00`, after the immutable product commit and its static read-back were available.
- Source state: PR #52 merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`; PR #62, PR #66, PR #67, PR #68, PR #70 and the intervening bounded improvements are merged through `main@333088147cdc77932efc41b90a08eb37e1c1cf42`. The complete executable PR #58 repair is bound to reachable two-parent product commit `c2e3aeea8d3cae18372b5ed99917eb95beb8d283`, tree `ec8702bc0f9350f90427a9d83a630105acb95dbe`, whose main-side parent is exact `33308814`; the final follow-up commit only binds documentation to that immutable source anchor. The integrated main side retains Desktop's shared 8–192 kHz import admission, mixes a pooled realtime PAD voice's terminal returned sample before retirement, and canonicalizes Android import display names to the archive's nonblank 240 UTF-16-unit ceiling before PCM publication. The prior built/runtime anchor remains `43d8ace6aa43f3eb6e3b9dc01ea74604ee600705`; its artifact receipts are not reused as proof for the repair.
...
- Runtime evidence at `43d8ace`: the exact final debug/test APKs passed seven API 36 instrumentation tests after data-preserving `install -r --no-streaming`. Fresh 100%, 200%, constrained portrait, 640 × 360 dp landscape at 130%/200%, scroll end and demo BEAT captures passed visual inspection. Large-text landscape stage rows measured 49 dp and the scroll-end demo target measured 59 dp. The exact packaged Windows EXE produced fresh CAPTURE and demo BEAT captures; B DRUMS / B-01 / 01–16 aligned and the exact tracked process tree exited cleanly. Those bytes predate `c2e3aee` and do not prove its gesture arbitration.
- Gate ceiling: the review repair is source/static-policy validated and awaits hosted CI; the prior exact anchor retains `LOCAL_PASS` plus scoped API 36 emulator runtime evidence only for its own bytes. The physical Pixel was not connected, so current physical `DEVICE_PASS`, touch feel, TalkBack speech, listening/latency, recording, route loss, provider, binary Release and `HUMAN_GO` are not inferred.
