...
The read-only preflight, idempotent provisioner, tracked-process launcher, and emulator-only matrix runner are now repository scripts. The runner refuses non-`emulator-*` serials and keeps physical Pixel evidence separate. Baseline smoke execution on API 36 passed the deterministic four-test suite in portrait at font scales 1.0, 1.3, and 2.0 and in landscape; after disabling emulator Bluetooth emulation it also passed three consecutive stability runs with no ChopLab fatal/ANR or Bluetooth fatal. A final source-bound run is required after the automation commit before promoting the current candidate's `COMPOSE_INSTRUMENTATION` / `FRAMEWORK_NODE` receipt.
