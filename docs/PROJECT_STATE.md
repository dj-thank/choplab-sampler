...
## Current snapshot — 2026-08-27 exact public ZIP content gate

Observed/read back at `2026-08-27T11:26:17+09:00`. Existing PR #69 head `1abbd8ff004edceafd81fe5ffa6059e2d3692d85` was merged locally with exact `main@0f5b672afb0e6b67e95290c31900ff5c8abc0ef4` as `fa4e6d2646e44039ce41662ba4c6ae6970ae9dd6`, tree `fafdc23a12c9e5f1e414d8aa2d57da676c382aff`. Product checkpoint is `3ed67101f362e61760c5ceb1839e71e91311aeb5`, tree `70e8f94f4e64cff4e13ea0e474e164632e500770`; product subtrees remain unchanged through the current policy repair.

- Product preservation: `app/`, `desktop/`, `shared/`, `jvm-core/`, `ios/` and build configuration match `main@0f5b672` exactly. The delta is release policy/tests/workflow wiring plus its plan.
...
- Resource repair: ZIP_LZMA dictionaries are capped at 16 MiB before `LZMADecompressor`; decoded text candidates share a 4 MiB archive-wide declared compressed-input budget in addition to the existing 512 KiB member output, 4 MiB metadata/output, 100:1 ratio and 4,096-entry bounds. Current-tree and repeatable explicit ZIP/APK roots additionally share 128-root / 128 MiB compressed / 128 MiB expanded work bounds. Excluded binary entries receive a bounded audio-signature probe before exclusion; APK-context binary bodies are fully verified and secret-scanned under 32 MiB/member and 64 MiB/APK bounds. Git evidence is streamed under output and ZIP-change-record caps; direct blob identification and non-commit tree listings have aggregate limits. `MemoryError` is fail-closed.
- Legitimate binary contract: `.lib` and exact JDK `runtime/lib/ct.sym` entries remain excluded as binary. A main `.app` executable up to the 4 MiB archive budget is fully decoded and secret-scanned. Larger arbitrary safe-named entries fail; only exact `runtime/lib/modules` receives a stored/deflate prefix probe bounded to 64 KiB/member and 256 KiB/archive, and it must expose a structurally consistent JIMAGE header. The exact root `icudtl.dat` in a bounded `skiko-awt-runtime-windows-*.jar` is capped at 16 MiB, structurally checked and fully decoded/secret-scanned; basename aliases get no exemption. Secret-shaped labels are redacted before findings reach logs.
- Nested/alias reach: ZIP-compatible nested archives are detected by bounded ZIP structure even behind a self-extracting prefix or neutral suffix and recurse to depth 3 under 64-archive, 16 MiB/member and shared 64 MiB compressed/expanded-work bounds; gzip/bzip2/xz/7z/tar signatures fail closed even when renamed. BOM-marked UTF-16/32 text and safe-named RIFF/AIFF/FLAC/Ogg/CAF/ID3/M4A plus ID3-less MPEG audio frames are detected, including audio hidden under `.bin`/other excluded suffixes. Raw central-directory Windows separators are rejected before platform path interpretation. Non-commit tree and direct ZIP-blob refs are enumerated and bounded.
- Exact consumer boundary: repeatable `--archive` inputs use the same ZIP/APK scanner. Desktop source snapshot, Android APK, Windows app-image ZIP and iOS Simulator ZIP are scanned after staging and before artifact upload. The release job scans all downloaded platform archives again before manifest, checksum, attestation or publication. A gap before an APK central directory is accepted only as a bounded, pair-structured `APK Sig Block 42`.
- Executable proof: complete Python policy 120 tests passes with one explicit Windows-only symlink-fixture skip on a host lacking privilege. Current/history public scans pass over 465 candidates. Configured validation passes 18 tasks; JVM 88 / 9 suites and Desktop 165 / 24 suites have zero failure/error/skip.
- Exact artifact controls: candidate source snapshot 1,542,548 / `F9B63B84A85A5D6336BE5C52FED5878DC6350AD20D09C3B3049015DA35C9B6A0`; existing signed Android APK 24,035,572 / `F8DCDBF5E7B13AF567F0388A5EFD885E61CFEA306F74AF590650DE677766772C`; current-main Windows app-image ZIP 89,156,340 / `7619DDE24822CC5CF6B38893382AC46DF8752AE777F046844E6322713F42AAA2`; current-main iOS Simulator ZIP 318,236 / `5D17C8BD5E3DC6C359FED40F1B79B38CD901D53343735566201AA454BB72475C`; all four pass together through the exact candidate scanner.
- Review/gate: local parent Standards/Spec unresolved `0/0`; original and later prefix/rename/blob-ref/audio/ICU/cross-candidate/history-enumeration, binary-audio, APK-boundary and Windows-separator cases no longer reproduce. Supported stored/deflate/BZIP2/default-LZMA and exact source/Android/Windows/iOS controls remain. Strongest fresh result for this unpushed repair is `LOCAL_PASS`; exact-head provider checks and review read-back must be refreshed on existing PR #69. No merge/tag/Release/device/provider-account/Human action is claimed by this snapshot.
