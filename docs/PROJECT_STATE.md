...
- Resource repair: ZIP_LZMA dictionaries are capped at 16 MiB before `LZMADecompressor`; decoded text candidates share a 4 MiB archive-wide declared compressed-input budget in addition to the existing 512 KiB member output, 4 MiB metadata/output, 100:1 ratio and 4,096-entry bounds. Current-tree and repeatable explicit ZIPs additionally share 128-root / 64 MiB compressed / 64 MiB expanded work bounds. Git evidence is streamed under output and ZIP-change-record caps; direct blob identification and non-commit tree listings have aggregate limits. `MemoryError` is fail-closed.
...
- Nested/alias reach: ZIP-compatible nested archives are detected by bounded ZIP structure even behind a self-extracting prefix or neutral suffix and recurse to depth 3 under 64-archive, 16 MiB/member and shared 64 MiB compressed/expanded-work bounds; gzip/bzip2/xz/7z/tar signatures fail closed even when renamed. BOM-marked UTF-16/32 text and safe-named RIFF/AIFF/FLAC/Ogg/CAF/ID3/M4A plus ID3-less MPEG audio frames are detected. Non-commit tree and direct ZIP-blob refs are enumerated and bounded.
- Exact consumer boundary: repeatable `--archive` inputs use the same scanner. Desktop source snapshot, Windows app-image ZIP and iOS Simulator ZIP are scanned after creation and before artifact upload. The release job scans downloaded Windows/iOS ZIPs again before manifest, checksum, attestation or publication.
- Executable proof: complete Python policy 116 tests passes with one explicit Windows-only symlink-fixture skip on a host lacking privilege. Current/history public scans pass over 465 candidates. Configured validation passes 18 tasks; JVM 88 / 9 suites and Desktop 165 / 24 suites have zero failure/error/skip.
- Exact artifact controls: candidate source snapshot 1,542,548 / `F9B63B84A85A5D6336BE5C52FED5878DC6350AD20D09C3B3049015DA35C9B6A0`; current-main Windows app-image ZIP 89,156,340 / `7619DDE24822CC5CF6B38893382AC46DF8752AE777F046844E6322713F42AAA2`; current-main iOS Simulator ZIP 318,236 / `5D17C8BD5E3DC6C359FED40F1B79B38CD901D53343735566201AA454BB72475C`; all three pass together through the exact candidate scanner.
- Review/gate: local parent Standards/Spec unresolved `0/0`; original and later prefix/rename/blob-ref/audio/ICU/cross-candidate/history-enumeration cases no longer reproduce, supported stored/deflate/BZIP2/default-LZMA and exact source/Windows/iOS controls remain. Strongest fresh result for this unpushed repair is `LOCAL_PASS`; exact-head provider checks and review read-back must be refreshed on existing PR #69. No merge/tag/Release/device/provider-account/Human action is claimed by this snapshot.
