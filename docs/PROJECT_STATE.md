...
- Immutable integration product: `1d0e850507e9e62d36cbffed8c20a3a7950931d1`, tree `f4f69b7498f8ccad7d91d1782f2d917b3497c1c8`. Exact-main, prior PR #58, PR #64/#76/#77 and Wave 10 heads are all ancestors; `git fsck --connectivity-only --no-dangling` and `git diff --check` pass. A later receipt-only commit may advance the branch without changing these product bytes.
...
- Current local read-back: the full 184-task Android/shared/JVM/Desktop lint, test, assembly and Windows packaging gate passes. XML reports contain 606 tests / 101 suites with failures, errors and skips all zero; Python policy has 64 passing tests; configured validation and both current-tree and reachable-history public-surface scans pass at 435 candidates. Android debug/release lint each report fatal 0 / error 0 / warning 7. CycloneDX 1.6 verifies `com.choplab:ChopLab:0.17.0` with 650 components / 651 dependencies.
- Artifact boundary: Android release metadata is `0.17.0` / code `27`. The final release APK passes manifest, permission/export, non-debuggable and alignment policy as an intentionally unsigned CI candidate; `--require-signed` rejects it. Windows ProductVersion `0.17.0` and packaged EXE read back successfully. No key, signer identity, certificate digest or secret was read or changed.
- Review and remote read-back: local Standards and Spec passes have unresolved findings 0 / 0. At `2026-08-27T01:25:02+09:00`, remote `main` remained `4f56a693`, PR #58 remained clean at `d56244aa`, repository rulesets were empty and the main protection endpoint reported unprotected. Fresh hosted workflows on the new exact head remain the merge gate.
