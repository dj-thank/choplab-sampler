...
- Legitimate binary contract: `.lib` and exact JDK `runtime/lib/ct.sym` entries remain excluded as binary. Large extensionless entries require a stored/deflate magic probe bounded to 64 KiB/member and 256 KiB/archive; known Mach-O/ELF/JIMAGE/ar formats pass, while unknown large safe-named text still fails. Secret-shaped labels are redacted before findings reach logs.
