...
- Observed at: `2026-08-24T22:14+09:00`.
- Source state: reachable policy product `7e98d9c8eddaa6ee28326bcf9e7ab579e6a90c1d`, tree `4d43ec3635797c064f2b4c787cd102f37ccec7d1`, advancing prior exact policy head `e827f48c8c0ffa48b6d04656bdc0785ac08fbc63` on squash-merged `main@a0b356c2e5820b7f9a8288ebcdd555c19e0cb6b5`; the later evidence commit is documentation-only. Merged #65/#68/#70/#71/#73 runtime, tests and documentation are retained.
- Archive policy: before `ZipFile` construction, a bounded tail read validates the end-of-central-directory record, rejects multi-disk/ZIP64 sentinels and any unclaimed prefix/trailing region, bounds declared central-directory bytes, and parses every fixed-size central record so a forged low end-record count cannot bypass the 4,096-entry cap. Each local filename, flag set, compression method and non-descriptor CRC/size tuple must agree with its central record before directory/binary classification or payload bounds are trusted. Non-binary ZIP members plus archive comments, central-directory filenames/comments/extra fields and local-header filenames/extra fields in both the current tree and reachable historical ZIP blobs are then scanned without extraction. Both metadata copies share the bounded archive budget, so a token present only in the local header cannot hide behind a safe central-directory value. ASCII credentials remain matchable beside malformed bytes through replacement decoding. Historical ZIP versions come from NUL-delimited per-parent raw path/blob change records before blob-ID deduplication, so newline-containing paths, merge-result-only archives and shared blobs with a non-ZIP path hint remain visible, while tree, symlink and gitlink objects never consume the blob cap. The scanner fails closed above 512 KiB per member, 4 MiB per archive or a 100:1 declared compression ratio; historical materialization is separately capped at 128 blobs, 16 MiB each and 64 MiB aggregate. Directory-marked entries with payload are rejected before content handling.
...
- Regression scope: UTF-8 BOM, control-containing and malformed-byte-adjacent secret text, secret-bearing member names, archive/member comments, central-directory extra fields, a local-header-only extra field paired with a safe central copy, central/local filename and size disagreement, token-bearing unclaimed prefix/trailing regions, newline-containing historical ZIP paths, reachable removed ZIP content whose `rev-list` hint names only a non-ZIP alias, merge-result-only ZIP discovery, ZIP-named tree/symlink exclusion, forged-low-count pre-construction entry rejection, member/aggregate/compression limits, payload-bearing directory entries and UTF-8-decodable raw-container exclusion are direct fixtures. Binary and audio member bodies containing token-shaped ASCII do not produce a content match.
- Fresh local checks: focused public-surface tests 26/26, complete Python policy suite 61/61, current-tree and reachable-history public scans over 395 candidates, conflict-marker scan and `git diff --check` PASS. Both known reachable historical ZIP blobs were included and passed. This Python-only delta requires fresh hosted CI; no Gradle, device, provider, public-release or Human result is claimed.
