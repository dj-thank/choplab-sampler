...
- Observed at: `2026-08-24T20:35:00+09:00`, after the immutable product commit and its static read-back were available.
- Source state: PR #52 merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`; PR #62, PR #66, PR #67, PR #68 and the intervening bounded improvements are merged through `main@2786c3722a9e56fa299d2a88f009d882545b0768`. The complete executable PR #58 repair is bound to reachable two-parent product commit `ba473e0ed3fc5c01caebc7183e2923476ae1c1c9`, tree `814e037746d9d73d0a8f90ccc6ce6a615377fc3d`, whose main-side parent is exact `2786c372`; the final follow-up commit only binds documentation to that immutable source anchor. The integrated main side retains Desktop's shared 8–192 kHz import admission, rejects 192,001 Hz before payload reads, and mixes a pooled realtime PAD voice's terminal returned sample before retirement. The prior built/runtime anchor remains `43d8ace6aa43f3eb6e3b9dc01ea74604ee600705`; its artifact receipts are not reused as proof for the repair.
...
- Shared navigation: `入れる → チョップ → ビート → 保存` remains the only workflow model. Font scale 1.2+ uses a simplified header, a two-by-two stage strip and multi-line status. CAPTURE scrolls for large text or compact landscape; large-text CHOP and BEAT bodies use explicit 48 dp-safe PAD heights and bounded scrolling so every control remains reachable. CHOP and ONE SHOT actions in those scroll bodies commit on completed tap, including an empty CHOP pad held stationary beyond the long-press timeout. A performance GATE waits through a 120 ms scroll-classification window, then consumes later movement and stays active until the physical pointer-up; a shorter completed tap receives an 80 ms minimum preview, and an older preview timer cannot release a newer retrigger. Parent cancellation before activation sends no PAD action, while pointer-node cancellation after activation releases trigger ownership exactly once. One-finger vertical movement over the CHOP waveform remains available to the parent scroller; horizontal intent and multi-touch retain waveform pan/zoom. Non-scroll PADs retain press-down response. Compact landscape keeps transient status feedback in the header when the separate status strip is absent.
...
- Review repair through `ba473e0`: all code-applicable GitHub findings are addressed without resolving their threads. Deferred GATE ownership releases on pointer-node cancellation; play-mode changes restart pointer arbitration; post-activation movement is consumed through physical up; stale preview generations cannot cut retriggers; short GATE taps retain at least 80 ms; and a held empty CHOP pad completes capture at pointer-up. The waveform detector now rejects one-finger vertical transform intent so the large-text CHOP scroller can move, while preserving horizontal and two-pointer transforms. Real-pointer regressions retain parent movement plus zero controller calls for CHOP, ONE SHOT and pre-activation GATE swipes, then add post-activation movement/no-scroll/no-release-until-up, overlapping short-preview generation ownership, and waveform-origin vertical scrolling. Computed PAD-grid heights retain 48 dp targets; normal-text fixed/responsive paths remain unchanged. Current-container checks pass `git diff --check`, 39 Python tests, parsing of all six Android XML files and the 394-candidate public-surface scan. Hosted Android run `32716871329` on prior head `18b815f` compiled product/tests and reached instrumentation, where the large-text target check correctly failed because it measured the inner `B01` text glyph rather than the PAD button. The integrated fixture measures the assigned PAD semantics node and separately asserts the full ONE SHOT/DRM description; fresh exact-head hosted CI is still required.
