...
This snapshot records a repository-policy hardening delta integrated with the main that merged reverse PAD resampling. It does not change product audio, UI, device, provider or release bytes.

- Observed at: `2026-08-24T23:06+09:00`.
- Source state: reachable integration product `92a28999992e8f5b74666a26a2e0a2a994b22ca5`, tree `337b9f722d7fde521e1d37cdcd6f11dfc44c6f30`, with parents repaired policy product `c4096092a0d000b50db702c9f5556c8227fa7ddd` and squash-merged `main@029500ac63fe521814530acf4d70cab78365c9fd`; the later evidence commit is documentation-only. The #74 reverse renderer, adapters, tests and documentation are retained.
- Archive policy: a bounded preflight parses the end record and every central/local record before `ZipFile` allocation. Every local header, metadata block, declared compressed span and optional data descriptor must form one contiguous ownership chain from byte zero to the central directory. Both 12-byte signatureless and 16-byte signed descriptors are compared with central CRC/sizes before record boundaries disambiguate them, including a signatureless CRC equal to the signature marker. Local/central filenames, flags, methods and non-descriptor CRC/sizes must agree before suffix or payload policy is trusted.
- Payload verification: a separate bounded raw decoder consumes the exact stored, deflate, BZIP2 or LZMA input span and requires exact decompressed size and CRC. It does not trust `ZipExtFile`'s declared-size truncation, so trailing compressed bytes and credential-shaped output beyond a forged short size fail closed. Safe-named member text and both metadata copies are scanned without extraction under 512 KiB/member and 4 MiB/archive/input bounds plus the 100:1 ratio cap.
- Repository reach: current symlink target text is scanned through `readlink` without traversal. NUL-delimited per-parent merge history enumerates every regular-file `.zip` path/blob version before deduplication, including newline paths, aliases and merge-result-only archives, while tree/symlink/gitlink objects do not consume the 128-blob / 16 MiB each / 64 MiB aggregate history caps. Binary/audio member bodies remain excluded and structured containers are not rescanned as raw UTF-8.
- Regression scope: forged-low entry counts, local/central metadata disagreement, unclaimed prefix/interior/trailing regions, descriptor disagreement and signature-marker CRC collision, compressed-stream trailing data, forged short decompressed output, all four supported compression methods, compressed-only directory payloads, current symlink targets and historical path/blob edge cases are direct fixtures.
- Fresh local checks: focused public-surface policy 33/33, complete Python policy 68/68, current/history scans over the integrated 396-candidate tree, conflict-marker scan and `git diff --check` PASS. Both known historical ZIP blobs were included. This Python-only delta requires fresh exact-head hosted CI; no Gradle, device, provider, public-release or Human result is claimed.
- Gate ceiling: source/static latest-main integration only. Hosted CI, clean exact-head review and merged-main read-back remain required; no administrator secret-scanning setting, binary Release, device or `HUMAN_GO` evidence is inferred.

## Previous snapshot — 2026-08-24 reverse PAD resampling tail merged-main read-back

This snapshot records one bounded JVM/Desktop PAD-rendering correction now squash-merged as PR #74. It changes reverse non-loop host PCM frame selection plus focused no-device regressions and preserves the pre-existing LOOP renderer/playhead contract; Android realtime code, forward playback, DSP controls, project schema and Java Sound resource ownership are unchanged.

- Observed at: `2026-08-24T23:06+09:00`.
- Source state: squash-merged `main@029500ac63fe521814530acf4d70cab78365c9fd`, tree `1ec604a2564ad39ff5c44d50f835d237c1ac6639`, from exact reviewed head `556cd0fa2c37950f349c970265a67afb82a3da36`; product `1b8b424036f76dabbb0671f4954ed079f32b9247`, tree `614c93f60c1e55e8df097302ef0e22250ddf7d37`. Renderer/test blobs are `ffd0be29db89d9ec75d553ab88c6d2d832e77e75` / `315ec9b078f9a8ed76f1783dce53877cbb430a53`; Desktop adapter/test blobs are `95a69a20e1e7c1667bcfa0f759d902eea73327fc` / `e4ccb83e52cdbde65f28d4cdf020cae7ffa72f2f`.
...
- Exact merge evidence: clean Codex review comment `5396238626`, zero unresolved threads, and successful Android `32735297596`, Windows `32735297567`, iOS `32735297469` and supply-chain `32735297580` workflows preceded expected-head squash merge.
- Gate ceiling: merged source plus exact-head hosted compilation/test evidence. Audible reverse-tail quality, physical Windows output, public artifacts and `HUMAN_GO` remain unclaimed.
