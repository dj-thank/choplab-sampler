...
- Source state: PR #52 merged at `main@495ddc9dfac02a9e72160c637f65d2b53d6829ce`; its follow-up PR binds the complete executable repair to reachable product commit `c650d00b51010afe62f74bad1d37f7afc1e79ed1`, tree `9e0470ad6e1811e93c3cfbdfd8645cb2519a6235`. Later commits on the PR only bind documentation to that immutable source anchor. The prior built/runtime anchor remains `43d8ace6aa43f3eb6e3b9dc01ea74604ee600705`; its artifact receipts are not reused as proof for the repair.
...
- Review repair through `c650d00`: all five GitHub findings are addressed—compact-landscape CAPTURE reachability, large-text BEAT quick/detail reachability, autosave-independent first-screen instrumentation, large-text compact-landscape CHOP dock reachability, and visible status feedback whenever its status strip is suppressed. Computed PAD-grid heights retain 48 dp targets; normal-text fixed/responsive paths remain unchanged. Current-container checks pass `git diff --check`, 23 Python tests, XML parsing and the 389-candidate public-surface scan. This Linux container has no Android SDK or cached Gradle distribution and cannot reach `services.gradle.org`; fresh hosted PR CI is required before promotion.
