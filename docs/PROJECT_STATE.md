...
- Exact JDK 17 local result for `9dc71a4`: shared Desktop 87, shared Android host 87, Android unit 288, JVM core 88; failures/errors/skips 0. Android lint, debug APK assembly, and AndroidTest Kotlin compilation also pass. An earlier uncommitted predecessor run exhausted host virtual memory in the default 4 GiB Gradle daemon; final `9dc71a4` passed with one worker, 1.5 GiB heap, and in-process Kotlin compilation. The crash is host-resource evidence, not a product-test result.
