...
1. scans the current tree and reachable history for secret-shaped content, signing material, and audio assets; current and historical ZIP blobs first bound central-directory bytes, parse the actual entry records, reject unclaimed prefix/trailing regions and require local filename/flags/compression/non-descriptor CRC-size agreement before parser allocation or suffix classification, then scan bounded non-binary member bytes plus archive comments, central-directory filenames/comments/extra fields, and local-header filenames/extra fields without extraction, enumerate regular-file `.zip` path/blob versions through NUL-delimited per-parent merge history before blob deduplication, fail closed above documented entry-count, member, archive, compression-ratio, historical-blob and historical-byte limits, and reject directory-marked payloads;
