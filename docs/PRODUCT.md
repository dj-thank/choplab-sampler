...
2026-09-25の訂正: 8月版の4工程・大きい正方形PAD・濃い波形面を土台に、BEAT右側の連続配置を組み込んだ案2の改訂版の見た目・操作感を維持します。下表は機能領域であり、5画面への再編を指定するものではありません。画面構成は [DESIGN](DESIGN.md) を優先します。歌唱・ミックス等の追加も既存の編集導線から扱います。
