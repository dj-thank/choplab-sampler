...
- A swipe may begin on a large-text CHOP/BEAT PAD without selecting, capturing or sounding it when the parent wins before activation. ONE SHOT and CHOP still commit on completed tap, while a held GATE preserves audible hold duration across separate trigger/release frames. Non-scroll performance latency is unchanged.
